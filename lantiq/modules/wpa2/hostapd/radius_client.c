...
extern int fromctrl_iface, glbllen;
extern char *glblbuf;
#define RADIUS_CLIENT_FIRST_WAIT 5 /* seconds */
#else
...
#ifdef HOSTAPD_OFFLOAD_RADIUS
    hostapd_ctrl_iface_radius_send((struct hostapd_data *)radius->ctx, 4, entry->msg->buf, entry->msg->buf_used);
#else
		       ...
		       const u8 *addr)
{
    ...
    /*
     * WASP - PB9x Offload Proxy Radius Configuration
     */
        hostapd_ctrl_iface_radius_send((struct hostapd_data *)radius->ctx, 4, msg->buf, msg->buf_used);
#else
