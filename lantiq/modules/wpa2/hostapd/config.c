        ...
        )
{
		...
		bss->ssid.ssid_len = strlen(value) -2;	/*avoid blank ssid, sunhanbiao 2012/7/3*/
		if (bss->ssid.ssid_len > HOSTAPD_MAX_SSID_LEN ||
			...
			if (!strcasecmp(value,"any")) {
				...
				memcpy(bss->ssid.ssid, value + 1,
					bss->ssid.ssid_len); /*avoid blank ssid, sunhanbiao 2012/7/3*/		
			}
	...
	if (bss->auth_algs == 0) {
        ...
        } else if (strcmp(tag, "wps_lock_pin") == 0) {		/*added by sunhanbiao 2012/8/15*/
                bss->wps->ap_setup_locked = (atol(value) == 1)?1:0;
