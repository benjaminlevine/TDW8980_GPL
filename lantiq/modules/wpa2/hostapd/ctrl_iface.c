...
#ifdef HOSTAPD_OFFLOAD_RADIUS
extern radius_client_receive(int sock, void *eloop_ctx, void *sock_ctx);
int fromctrl_iface = 0, glbllen = 0;
char *glblbuf;
#endif
...
#ifdef HOSTAPD_OFFLOAD_RADIUS
    else if (strncmp(buf, "RADIUS", 6) == 0) {
        index = (char *)&buf[0]; 
        index = index + strlen(signature) + 1;
        res = res - strlen(signature) + 1;
        fromctrl_iface = 1;
        glbllen = res;
        glblbuf = malloc(res);
        memcpy(glblbuf, index, res);

        radius_client_receive(sock, hapd->radius, (void *) RADIUS_AUTH);
...
void hostapd_ctrl_iface_radius_send(struct hostapd_data *hapd, int level,
			     char *buf, size_t len)
{
	...
	snprintf(levelstr, sizeof(levelstr), "%s", signature);
	...
	while (dst) {
		...
		if (level >= dst->debug_level) {
			...
			if (sendmsg(hapd->ctrl_sock, &msg, 0) < 0) {
				...
				dst->errors++;
				if (dst->errors > 10) {
                    printf("hostapd_ctrl_iface_detach \n");
