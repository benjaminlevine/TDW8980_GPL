...
static int hostapd_ctrl_iface_wps_pin(struct hostapd_data *hapd, char *txt)
{
	...
	wpa_printf(MSG_DEBUG, "hostapd_ctrl_iface_wps_pin %s", txt);
	if (pin == NULL)
	{
		wpa_printf(MSG_DEBUG, "pin == NULL");
		return -1;
	}
	*pin++ = '\0';
...
#endif /* CONFIG_IEEE80211W */
#ifdef CONFIG_WPS
	} else if (os_strncmp(buf, "CONFIGTHEM ", 11) == 0) {
		if (!hostapd_ctrl_iface_wps_pin(hapd, buf + 11))
			reply_len = -1;
	} else if (os_strncmp(buf, "WPS_PIN ", 8) == 0) {
		if (hostapd_ctrl_iface_wps_pin(hapd, buf + 8))
			reply_len = -1;
	} else if (os_strcmp(buf, "WPS_PBC") == 0 || os_strncmp(buf, "CONFIGTHEM", 10) == 0) {
