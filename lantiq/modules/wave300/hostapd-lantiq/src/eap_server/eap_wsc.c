			    ...
			    struct wpabuf *respData)
{
	...
	wpa_printf(MSG_DEBUG, "eap_serv : this serv mode\n");

	eloop_cancel_timeout(eap_wsc_ext_reg_timeout, sm, data);
	if (data->ext_reg_timeout) {
		write_wps_status_value(WPS_STATUS_OVERLAP);
		write_wps_led_value(WPS_LED_OVERLAP); /* added by wwj 121213 */
		eap_wsc_state(data, FAIL);
		return;
	}
	...
	case WPS_DONE:
		...
		write_wps_led_value(WPS_LED_SUCCESS); /* added by wwj 121213 */
	...
	case WPS_CONTINUE:
		...
		write_wps_led_value(WPS_LED_INPROGRESS);/* added by wwj 121213 */
		eap_wsc_state(data, MSG);
		break;
	case WPS_FAILURE:
		write_wps_status_value(WPS_STATUS_ERROR);
		write_wps_led_value(WPS_LED_ERROR);/* added by wwj 121213 */
