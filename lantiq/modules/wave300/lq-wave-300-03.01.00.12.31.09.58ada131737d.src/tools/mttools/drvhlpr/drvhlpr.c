...
_signal_thread_proc (void* param)
{
  while (1) {
    ...
    if (mem_alarm_fired) {
      ...
      case MAT_LAST:
      default:
        WARNING("Incorrect Memory Alarm Type: %d", mem_alarm_type);
        break;
      }
    }
    mtlk_osal_msleep(20);
                  ...
                  uint32        allocated)
{
  ...
  ERROR("Memory alarm limit reached: %d >= %d", allocated, mem_alarm_limit);
...
load_cfg (const char *cfg_fname)
{
  ...
  res = iniparser_aux_get_str(dict,
                              iniparser_aux_gkey_const("interface"),
                              wlanitrf,
                              sizeof(wlanitrf));
  if (res != MTLK_ERR_OK) {
    ERROR("'interface' parameter reading error: %d", res);
    goto end;
  }

  net_type = iniparser_getstr(dict, iniparser_aux_gkey_const("network_type"));
  if (!net_type) {
    ERROR("'network_type' parameter reading error");
    goto end;
  }
  ...
  if (!tmp) {
    ERROR("Can't find the 'arp_iface0' parameter");
    goto end;
  }

  res = arp_helper_register_iface(tmp);
  if (res != MTLK_ERR_OK) {
    ERROR("Can't register ARP xFace#0 (%s), err=%d'", tmp, res);
    res = MTLK_ERR_PARAMS;
    goto end;
  }
  ...
  if (tmp) {
    res = arp_helper_register_iface(tmp);
    if (res != MTLK_ERR_OK) {
      ERROR("Can't register ARP xFace#1 (%s), err=%d'", tmp, res);
      res = MTLK_ERR_PARAMS;
      goto end;
    }
  }
  ...
  if (!tmp) {
    LOG0("'wps_script_path' parameter isn't present");
    goto end;
  }
  ...
  if (res != MTLK_ERR_OK) {
    ERROR("'wps_script_path' parameter set error #%d", res);
    goto end;
  }
  ...
  if (res != MTLK_ERR_OK) {
    ERROR("'network_type' parameter set error #%d", res);
    goto end;
  }

  tmp = iniparser_getstr(dict, iniparser_aux_gkey_const("wls_link_script_path"));
  if (!tmp) {
    ERROR("'wls_link_script_path' parameter reading error");
    goto end;
  }

  res = leds_ctrl_set_link_path(tmp);
  if (res != MTLK_ERR_OK) {
    ERROR("'wls_link_script_path' parameter set error #%d", res);
    goto end;
  }
  ...
  if (!tmp) {
    ERROR("'wls_link_status_script_path' parameter reading error");
    goto end;
  }

  res = leds_ctrl_set_link_status_path(tmp);
  if (res != MTLK_ERR_OK) {
    ERROR("'wls_link_script_path' parameter set (LEDS) error #%d", res);
    goto end;
  }

  res = wps_ctrl_set_link_status_path(tmp);
  if (res != MTLK_ERR_OK) {
    ERROR("'wls_link_script_path' parameter set (WPS) error #%d", res);
    goto end;
  }
...
_uninstall_sigactions (void)
{
  ...
  if (res != 0) {
    ERROR("return error from sigaction: %d", errno);
    MTLK_ASSERT(0);
  }
}
...
_install_sigactions (void)
{
  ...
  if (res != 0) {
    ERROR("return error from pthread_sigmask: %d", errno);
    res = MTLK_ERR_PARAMS;
    goto end;
  }
...
_print_help (const char *app_name)
{
  ...
  mtlk_argv_print_help(stdout,
                       app_fname,
                       "Metalink Driver Helper v." MTLK_PACKAGE_VERSION,
                       all_params,
...
process_commandline (int argc, char *argv[])
{
  ...
  if (res != MTLK_ERR_OK) {
    ERROR("Can't init argv parser (err=%d)", res);
    goto end;
  }
  inited = TRUE;
  ...
  if (!params_conf_file) {
    ERROR("CFG file must be specified!");
    res = MTLK_ERR_PROHIB;
    goto end;  
  }
...
static void __MTLK_IFUNC 
main_irb_handler (mtlk_handle_t      context,
                  ...
                  uint32            *size)
{
  if (mtlk_guid_compare(evt, &IRBE_HANG) == 0) {
    ...
    ERROR("Unexpected GUID: %s", guid_str);
  }
}
...
MTLK_INIT_STEPS_LIST_BEGIN(drvhlpr_main)
  ...
  MTLK_INIT_STEPS_LIST_ENTRY(drvhlpr_main, IRBM_INIT)
  MTLK_INIT_STEPS_LIST_ENTRY(drvhlpr_main, IRB_INIT)
  MTLK_INIT_STEPS_LIST_ENTRY(drvhlpr_main, IRBM_START)
...
_connect_to_drv (void)
{
  ...
  LOG_LED("Connected to driver, interface name = %s", wlanitrf);
...
_drvhlpr_register_components (drvhlpr_main_t *_drvhlpr)
{
  ...
  for (i = 0; i < ARRAY_SIZE(drvhelper_components); ++i) {
    ...
    LOG0("Component added: %s", drvhelper_components[i].name);
  }
...
_drvhlpr_main_cleanup (drvhlpr_main_t *_drvhlpr)
{
  ...
  MTLK_CLEANUP_BEGIN(drvhlpr_main, MTLK_OBJ_PTR(_drvhlpr))
    ...
    MTLK_CLEANUP_STEP(drvhlpr_main, IRB_REG, MTLK_OBJ_PTR(_drvhlpr),
                      mtlk_irb_unregister, (_drvhlpr->main_irb_client));
    ...
    MTLK_CLEANUP_STEP(drvhlpr_main, IRBM_START, MTLK_OBJ_PTR(_drvhlpr),
                      mtlk_irb_media_stop, ());
    MTLK_CLEANUP_STEP(drvhlpr_main, IRB_INIT, MTLK_OBJ_PTR(_drvhlpr),
                      mtlk_irb_cleanup, ());
    MTLK_CLEANUP_STEP(drvhlpr_main, IRBM_INIT, MTLK_OBJ_PTR(_drvhlpr),
...
_drvhlpr_main_init (drvhlpr_main_t *_drvhlpr, int argc, char *argv[])
{
  ...
  MTLK_INIT_TRY(drvhlpr_main, MTLK_OBJ_PTR(_drvhlpr))
    ...
    MTLK_INIT_STEP(drvhlpr_main, CONNECT_TO_DRV, MTLK_OBJ_PTR(_drvhlpr),
                   _connect_to_drv, ());
    MTLK_INIT_STEP(drvhlpr_main, IRBM_INIT, MTLK_OBJ_PTR(_drvhlpr),
                   mtlk_irb_media_init, ());
    MTLK_INIT_STEP(drvhlpr_main, IRB_INIT, MTLK_OBJ_PTR(_drvhlpr),
                   mtlk_irb_init, ());
    MTLK_INIT_STEP(drvhlpr_main, IRBM_START, MTLK_OBJ_PTR(_drvhlpr),
    ...
    LOG0("MTLK CLI Server (%s) bound on port %d", cli_srv_name, tmp);
    ...
    MTLK_INIT_STEP_EX(drvhlpr_main, IRB_REG, MTLK_OBJ_PTR(_drvhlpr),
                      mtlk_irb_register, (MAIN_IRBE_LIST, 
                                          ARRAY_SIZE(MAIN_IRBE_LIST), 
                                          main_irb_handler, 
                                          0),
                      _drvhlpr->main_irb_client,
                      _drvhlpr->main_irb_client != HANDLE_T(0),
                      MTLK_ERR_NO_MEM);
