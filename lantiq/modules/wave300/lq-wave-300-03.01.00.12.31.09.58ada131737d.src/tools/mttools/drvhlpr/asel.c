             ...
             rf_mgmt_data_t                     rf_mgmt_data)
{
  ...
  if (peer_info->best_data_idx[rank_idx] == LBF_BEST_DATA_IDX_NONE) {
    ...
    data_idx  = _lbf_get_rank_best_data_idx(lbf, 
                                            ...
                                            rank_idx);
    best_in_rank = TRUE;
    LOG1("Best on rank (enum): %d", data_idx);
  }
  else {
...
_mtlk_rf_mgmt_lbf_constructor (const struct mtlk_rf_mgmt_cfg *cfg)
{
  ...
  if (lbf) {
    ...
    if (res != MTLK_ERR_OK) {
      ERROR("Can't init LBF object (err=%d)", res);
                              ...
                              rf_mgmt_data_t               *rf_mgmt_data)
{
  ...
  if (spr_hdr->u16Version != SPR_DATA_VERSION_2) {
    WARNING("SPR of unsupported version received (%d != %d)", 
            spr_hdr->u16Version,
            SPR_DATA_VERSION_2);
    goto end;
  }
  ...
  if (spr_hdr->u16SPDataSize != sizeof(*sp)) {
    ERROR("Incorrect SP Data size (%d != %d)",
  ...
  if (spr_hdr->u16MetricsSize != sizeof(*m)) {
    ERROR("Metrics of incorrect size (%u != %d)",
  ...
  LOG0("SPR: id=%u esnr=%u, mcsf=%d esnr_db=%u", 
                  ...
                  enum _asel_gt_decision_code        *_dcode)
{
  ...
  if (data_info_2->average_esnr >= data_info_1->average_esnr) {
    ...
    LOG4("R=%d A1=%d A2=%d: ESNR2 (%d) >= ESNR1 (%d):", 
    ...
    */
    ERROR("R=%d A1=%d A2=%d: MCSF2 (%d, ESNR=%d) > MCSF1 (%d, ESNR=%d)!",
      rank_idx, data_idx_1, data_idx_2, 
              ...
              rf_mgmt_data_t                      rf_mgmt_data)
{
  ...
  if (best_in_rank) {
    LOG1("R=%d A=%d: Best on Rank (Aprev=%d)",  
      rank_idx, data_idx, rank_idx, peer_info->best_data_idx[rank_idx]);
#ifdef MTCFG_DEBUG
    if (debug) {
    ...
    if (_asel_is_all_ranks_best(asel, peer_info, rank_idx)) {
      LOG1("R=%d D=%d: Best (Aprev=%d)",  rank_idx, all_g2_antenna_sets[data_idx], rf_mgmt_data);
      peer_info->current_data_idx = data_idx;
    }
    else {
      uint16 s_rank_idx = _RF_MGMT_GET_S_RANK_IDX(rank_idx);
      LOG1("Sup R=%d D=%d: Best", s_rank_idx, all_g2_antenna_sets[peer_info->best_data_idx[s_rank_idx]]);
...
_mtlk_rf_mgmt_asel_constructor (const struct mtlk_rf_mgmt_cfg *cfg)
{
  ...
  if (asel) {
    ...
    if (res != MTLK_ERR_OK) {
      ERROR("Can't init ASEL object (err=%d)", asel);
                               ...
                               rf_mgmt_data_t               *rf_mgmt_data)
{
  ...
  if (spr_hdr->u16Version != SPR_DATA_VERSION_1) {
      WARNING("SPR of unsupported version received (%d)", 
        spr_hdr->u16Version);
      goto end;
  }
  ...
  LOG0("SPR: id=%u data_idx=%d rank_idx=%d esnr=%u, mcsf=%d shcp=%u", 
...
static __INLINE uint32
rf_mgmt_hash_hash_func (const struct eth_addr *key)
{
                        ...
                        rf_mgmt_data_t          desired_rf_mgmt_data)
{
  ...
  res = mtlk_irb_call_drv(&IRBE_RF_MGMT_SET_PEER_DATA, &peer_data, sizeof(peer_data));
  if (res != MTLK_ERR_OK)  {
    WARNING("Can't set peer RF Mgmt Data (res=%d)", res);
  }
  else if (peer_data.result != MTLK_ERR_OK) {
    WARNING("Can't set peer RF Mgmt Dat (irb_res=%d)", peer_data.result);
                    ...
                    struct mtlk_rf_mgmt_evt_get_spr *spr_info)
{
  ...
  if (h) {
    ...
    peer_node = 
      (struct mtlk_rf_mgmt_peer_node *)mtlk_osal_mem_alloc(peer_node_size, 
                                                           MTLK_MEM_TAG_RFMGMT);
    if (!peer_node) {
      ERROR("Can't allocate RF MGMT peer node!");
      goto end;
    }
  ...
  res = rf_mgmt->api->handle_spr(rf_mgmt->api_ctx,
                                 ...
                                 &new_rf_mgmt_data);
  if (res != MTLK_ERR_OK) {
    ERROR("SPR handling failed (err=%d)", res);
    goto end;
  }
  ...
  res = _rf_mgmt_set_peer_data(rf_mgmt,
                               ...
                               new_rf_mgmt_data);
  if (res != MTLK_ERR_OK) {
    ERROR("Can't set peer RF MGMT data (err=%d)", res);
  }

  LOG0("Peer (" MAC_FMT ") : RF Mgmt data changed: 0x%02x => 0x%02x",
       MAC_ARG(spr_info->mac), 
...
_rf_mgmt_get_and_handle_spr (struct mtlk_rf_mgmt *rf_mgmt)
{
  ...
  res = mtlk_irb_call_drv(&IRBE_RF_MGMT_GET_SPR, 
                          spr_info, 
                          __rf_mgmt_sizeof_spr_evt_buff(rf_mgmt));
  if (res != MTLK_ERR_OK || spr_info->result != MTLK_ERR_OK) {
    WARNING("Can't get SPR (res=%d, irb_res=%d)", res, spr_info->result);
    goto end;
  }

  LOG2("SPR data received");
...
_rf_mgmt_send_sp (struct mtlk_rf_mgmt *rf_mgmt)
{
  ...
  LOG0("SP: id=%d aset=0x%02x rank=%d", 
    rf_mgmt->sp_id, sp_info->rf_mgmt_data, sp_info->rank);

  ++rf_mgmt->sp_id;

  res = mtlk_irb_call_drv(&IRBE_RF_MGMT_SEND_SP, 
                          sp_info, 
                          __rf_mgmt_sizeof_sp_evt_buff(rf_mgmt));
  if (res != MTLK_ERR_OK || sp_info->result != MTLK_ERR_OK) {
    WARNING("Can't send SP (res=%d, irb_res=%d)", res, sp_info->result);
  }
}
...
_rf_mgmt_get_type (uint8 *type)
{
  ...
  res = mtlk_irb_call_drv(&IRBE_RF_MGMT_GET_TYPE, &evt, sizeof(evt));
  evt.type.u16Status = MAC_TO_HOST16(evt.type.u16Status);
  if (res != MTLK_ERR_OK) {
    WARNING("Can't get type (res=%d)", res);
  }
  else if (evt.result != MTLK_ERR_OK) {
    WARNING("Can't get type (irb_res=%d)", (int)evt.result);
    res = (int)evt.result;
  }
  else if (evt.type.u16Status != UMI_OK) {
    WARNING("Can't Enable (mac_res=%d)", (int)evt.type.u16Status);
    res = MTLK_ERR_MAC;
  }
  else {
...
_rf_mgmt_set_type (uint8 type)
{
  ...
  LOG0("Sending Type (%d)", type);

  res = mtlk_irb_call_drv(&IRBE_RF_MGMT_SET_TYPE, &evt, sizeof(evt));
  evt.type.u16Status = MAC_TO_HOST16(evt.type.u16Status);
  if (res != MTLK_ERR_OK) {
    WARNING("Can't send Enable (res=%d)", res);
  }
  else if (evt.result != MTLK_ERR_OK) {
    WARNING("Can't Enable (irb_res=%d)", (int)evt.result);
    res = (int)evt.result;
  }
  else if (evt.type.u16Status != UMI_OK) {
    WARNING("Can't Enable (mac_res=%d)", (int)evt.type.u16Status);
...
_rf_mgmt_get_def_data (uint8 *rf_mgmt_data)
{
  ...
  res = mtlk_irb_call_drv(&IRBE_RF_MGMT_GET_DEF_DATA, &evt, sizeof(evt));
  if (res != MTLK_ERR_OK) {
    WARNING("Can't get def data (res=%d)", res);
  }
  else if (evt.result != MTLK_ERR_OK) {
    WARNING("Can't get def data (irb_res=%d)", (int)evt.result);
    res = (int)evt.result;
  }
  else if (evt.data.u8Status != UMI_OK) {
    WARNING("Can't get data (mac_res=%d)", (int)evt.data.u8Status);
    res = MTLK_ERR_MAC;
  }
  else {
...
_rf_mgmt_set_def_data (uint8 rf_mgmt_data)
{
  ...
  res = mtlk_irb_call_drv(&IRBE_RF_MGMT_SET_DEF_DATA, &evt, sizeof(evt));
  if (res != MTLK_ERR_OK) {
    WARNING("Can't set def data (res=%d)", res);
  }
  else if (evt.result != MTLK_ERR_OK) {
    WARNING("Can't set def data (irb_res=%d)", (int)evt.result);
    res = (int)evt.result;
  }
  else if (evt.data.u8Status != UMI_OK) {
    WARNING("Can't set data (mac_res=%d)", (int)evt.data.u8Status);
    res = MTLK_ERR_MAC;
  }
...
_rf_mgmt_thread_proc (mtlk_handle_t context)
{
  ...
  for (count = 0; count < RF_MGMT_MAX_ENABLE_ATTEMPTS; ++count) {
    if (rf_mgmt->stop_signaled) {
      LOG0("Stop signalled! Exiting thread.");
      break;
    }
  ...
  while (!error) {
    ...
    case MTLK_ERR_TIMEOUT: /* timeout */
      ...
      break;
    default:
      ERROR("Unexpected Evt wait result %d. Exiting thread.", wait_res);
      error = TRUE;
      break;
    }
...
static void __MTLK_IFUNC
_rf_mgmt_irbh_spr_arrived (mtlk_handle_t      context,
...
_rf_mgmt_cleanup (struct mtlk_rf_mgmt *rf_mgmt)
{
  MTLK_STOP_BEGIN(rf_mgmt,  MTLK_OBJ_PTR(rf_mgmt))
    ...
    MTLK_STOP_STEP(rf_mgmt, IRB_REGISTER_ON_SPR, MTLK_OBJ_PTR(rf_mgmt),
                   mtlk_irb_unregister, (rf_mgmt->spr_irb_ctx));
               ...
               const struct mtlk_rf_mgmt_cfg *cfg)
{
  ...
  MTLK_START_TRY(rf_mgmt,  MTLK_OBJ_PTR(rf_mgmt))
    ...
    MTLK_START_STEP_EX(rf_mgmt, IRB_REGISTER_ON_SPR, MTLK_OBJ_PTR(rf_mgmt),
                       mtlk_irb_register, (&IRBE_RF_MGMT_SPR_ARRIVED, 
                                           1,
                                           _rf_mgmt_irbh_spr_arrived, 
                                           HANDLE_T(rf_mgmt)),
                       rf_mgmt->spr_irb_ctx,
                       rf_mgmt->spr_irb_ctx != HANDLE_T(0),
                       MTLK_ERR_NO_RESOURCES);
                 ...
                 const struct mtlk_rf_mgmt_cfg *cfg)
{
  ...
  if (rf_mgmt != NULL) {
    ...
    if (_rf_mgmt_init(rf_mgmt, api, cfg) == MTLK_ERR_OK) {
      LOG0("RF MGMT (type#%d) initialized", (int)cfg->rf_mgmt_type);
    }
    else {
      ERROR("Can't start RF MGMT (type#%d)", (int)cfg->rf_mgmt_type);
      mtlk_osal_mem_free(rf_mgmt);
      rf_mgmt = NULL;
    }
  }
  else {
    ERROR("Can't allocate RF MGMT object (type#%d) of %d bytes", 
                                    ...
                                    uint32                    nof_vals)
{
  ...
  if (nof_params != nof_vals) {
    ERROR("Wrong number of parameters: %u (%u expected)", nof_params, nof_vals);
    res = MTLK_ERR_PARAMS;
    goto end;
  }
                                       ...
                                       int32                     max_val)
{
  ...
  for (i = 0; i < nof_vals; i++) {
    if (vals[i] < min_val || vals[i] > max_val) {
      ERROR("Parameter#%u not in range [%d...%d]", i, min_val, max_val);
      res = MTLK_ERR_PARAMS;
      goto end;
    }
  }
                                   ...
                                   mtlk_handle_t             ctx)
{
  ...
  if (res != MTLK_ERR_OK) {
    ERROR("Can't add response string");
    goto end;
  }
                                  ...
                                  mtlk_handle_t             ctx)
{
  ...
  res = mtlk_irb_call_drv(&IRBE_RF_MGMT_SEND_SP, 
                          ...
                          sizeof(dbg_sp));
  if (res != MTLK_ERR_OK) {
    ERROR("Can't send SP (res=%d, irb_res=%d)", res);
  }
  else if (dbg_sp.hdr.result != MTLK_ERR_OK) {
    ERROR("Can't send SP (irb_res=%d)", res, dbg_sp.hdr.result);
                                   ...
                                   mtlk_handle_t             ctx)
{
  ...
  res = mtlk_irb_call_drv(&IRBE_RF_MGMT_GET_SPR, 
                          ...
                          sizeof(dbg_spr));
  if (res != MTLK_ERR_OK) {
    WARNING("Can't get SPR (res=%d)", res);
    goto end;
  }

  if (dbg_spr.hdr.result != MTLK_ERR_OK) {
    WARNING("Can't get SPR (irb_res=%d)", (int)dbg_spr.hdr.result);
  ...
  case SPR_DATA_VERSION_1:
    if (spr_hdr->u16MetricsSize == sizeof(ASL_SHRAM_METRIC_T)) {
      ...
      WARNING("Metrics of incorrect size (%u != %d)",
              spr_hdr->u16MetricsSize,
  ...
  mtlk_cli_srv_rsp_add_strf(rsp, MAC_FMT ",%u,%u,%u,%u,%u,%u,%u,%u,%u",
                            MAC_ARG(dbg_spr.hdr.mac),
                            sp_data->id,
...
_rf_mgmt_dbg_create (void)
{
  ...
  if (rf_mgmt_dbg != NULL) {
    ...
    if (_rf_mgmt_dbg_init(rf_mgmt_dbg) == MTLK_ERR_OK) {
      LOG0("DBG RF MGMT initialized!");
    }
    else {
      ERROR("Can't start DBG RF MGMT");
      mtlk_osal_mem_free(rf_mgmt_dbg);
      rf_mgmt_dbg = NULL;
    }
  }
  else {
    ERROR("Can't allocate DBG RF MGMT object of %d bytes", sizeof(*rf_mgmt_dbg));
  }
...
rf_mgmt_start (void)
{
  ...
  case MTLK_RF_MGMT_TYPE_AIRGAIN_ASEL:
    ...
    break;
  default:
    WARNING("Unsupported RF Management type: %d. Forcing TYPE_OFF", 
