...
wps_thrdatapipe (mtlk_handle_t context)
{
  ...
  */
  while (stop_thread == 0) {
    ...
    if(pipe_rslt==-1)
    {
      LOG_LED("failed to open %s pipe file for reading, try agin\n", leds_fifo_file);
      goto end;
    }
    else 
    {
      ...
      for (;;) {
        ...
        if (read_count < 0) {
          ERROR("failed to read pipe file\n");
          close(pipe_rslt);
          goto end;
        }
        ...
        if (read_count == 0) {
          ...
          if (script) {
            ...
            pid = fork();
            if (pid == 0) {
              LOG_LED("fork: write %d to %s script",wps_status,wps_script_path);
...
create_wps_task(void)
{
  ...
  if (ret_val != MTLK_ERR_OK) {
    ERROR("wps_thread init failed (err=%d)", ret_val);
  ...
  ret_val = mtlk_osal_thread_run(&wps_thread,
                                 ...
                                 HANDLE_T(0));
  if (ret_val != MTLK_ERR_OK) {
    ERROR("wps_thread start failed (err=%d)", ret_val);
...
wps_ctrl_start (void)
{
  ...
  LOG_LED("set receive signal with SIGUSR1 and SIGUSR2 signals");
  ...
  if (sigaction (SIGUSR1, &setup_action, NULL) != 0) {
    ERROR("return error from sigaction ");
    goto end;
  }
...
{
  led_resolution = val;
  LOG_LED("wps API: led_resolution = %d",led_resolution);
}
...
int wps_ctrl_set_link_status_path(char* str)
{
  ...
  if (size <= sizeof(link_status_script_path)) {
    ...
    LOG_LED("wps API: link_status_script_path = %s",link_status_script_path);
    return MTLK_ERR_OK;
  }
...
int wps_ctrl_set_wps_script_path(char* str)
{
  ...
  if (size <= sizeof(wps_script_path)) {
    ...
    LOG_LED("wps API: wps_script_path = %s",wps_script_path);
    return MTLK_ERR_OK;
  }
...
leds_signal(int s) 
{
  ...
  LOG_LED("catch the signal, count = %d, signal = %d",open_file, s);
  if (s == 10) {
    /*on first entry close the file*/
    LOG_LED("on first entry close the file");
    close(pipe_rslt);
    open_file = 0;
  }
  else {
