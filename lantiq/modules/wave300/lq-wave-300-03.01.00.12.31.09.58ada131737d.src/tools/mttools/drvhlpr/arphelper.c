...
cmd_do_arp(struct mtlk_arp_data *parp_data, const char *iface)
{
  ...
  if (strlen(iface) > sizeof(ifr.ifr_name)) {
    ERROR("Interface \"%s\" name is too long (maximum %d)",
  ...
  if (ioctl(arp_sock_fd, SIOCGIFINDEX, &ifr) < 0) {
    ERROR("SIOCGIFINDEX: %s", strerror(errno));
    retval = -1;
    goto end;
  }
  ...
  if (sendto(arp_sock_fd, &pkt, sizeof(pkt), 0, (struct sockaddr *) &ll,
               sizeof(ll)) < 0) {
    ERROR("While sending ARP packet: %s", strerror(errno));
    retval = -1;
    goto end;
  }
...
_irb_arp_req_handler (mtlk_handle_t      context,
                      ...
                      uint32            *size)
{
  ...
  if (!arp_iface_lst) {
    WARNING("No interfaces to send ARP request to.");
  }

  for (aif = arp_iface_lst; aif; aif = aif->next) {
    LOG1("ARP sending to %s", aif->name);
    if (0 != cmd_do_arp(arp_data, aif->name)) {
      WARNING("Send ARP to %s failed", aif->name);
    }
  }
}
...
arp_helper_start (void)
{
  ...
  if (arp_sock_fd < 0) {
    ERROR("Cannot open ARP packet interface: %s", strerror(errno));
    goto socket_open_failed;
  }

  res = mtlk_irb_register(&IRBE_ARP_REQ, 1, _irb_arp_req_handler, 0);
  if (!res) {
    ERROR("Cannot register IRB handler");
