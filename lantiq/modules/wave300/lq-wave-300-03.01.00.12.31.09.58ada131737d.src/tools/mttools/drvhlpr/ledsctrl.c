...
load_security_cfg (const char *cfg_fname)
{
  ...
  if (!dict) {
    ERROR("Can't load CFG file (%s)", cfg_fname);
    res = MTLK_ERR_PARAMS;
    goto end;
  }
  ...
  security_mode = iniparser_getint(dict, 
                                   ...
                                   security_mode);
  LOG_LED("security_mode = %d",security_mode);
  res  = MTLK_ERR_OK;
end:
  if (dict) {
...
prepare_leds (void)
{
  ...
  if (leds_parse_configs() != 0) {
    ERROR("return with error from leds_parse_configs()");
    goto end;
  }
...
create_led_ctrl_task (void)
{
  ...
  if (ret_val != MTLK_ERR_OK) {
    ERROR("ledsctrl_thread stop event init failed! (err=%d)", ret_val);
    goto evt_init_failed;
  }

  ret_val = mtlk_osal_thread_init(&ledsctrl_thread);
  if (ret_val != MTLK_ERR_OK) {
    ERROR("ledsctrl_thread init failed! (err=%d)", ret_val);
  ...
  ret_val = mtlk_osal_thread_run(&ledsctrl_thread,
                                 ...
                                 HANDLE_T(0));
  if (ret_val != MTLK_ERR_OK) {
    ERROR("ledsctrl_thread start failed! (err=%d)", ret_val);
...
{
  LOG1("call pthread_cancel to close led thread\n");
...
static int map_str_to_code(char *ptr, const char *value)
{
  if (strcmp("led_off", value) == 0) {
    ...
    ERROR("ERROR, param %s was not found in drvhlpr.conf",value);
...
static void set_error_array(const char *key, const char *value)
{
  if (strcmp("wps_error_overlap_err", key)==0) {
    ...
    LOG_LED("wps_errors[MTLK_WPS_STOP_OVERLAP] = %s",wps_errors[MTLK_WPS_STOP_OVERLAP]);
  }
  if (strcmp("wps_error_timeout_err", key)==0) {
    wps_errors[MTLK_WPS_TIMEOUT][0] = wps_error_timeout_err[0];
    LOG_LED("wps_errors[MTLK_WPS_TIMEOUT] = %s",wps_errors[MTLK_WPS_TIMEOUT]);
  }
  if (strcmp("wps_error_stop_pin_err", key)==0) {
    wps_errors[MTLK_WPS_STOP_PIN_ERROR][0] = wps_error_stop_pin_err[0];
    LOG_LED("wps_errors[MTLK_WPS_STOP_PIN_ERROR] = %s",wps_errors[MTLK_WPS_STOP_PIN_ERROR]);
  }
  if (strcmp("wps_error_stop_err", key)==0) {
    wps_errors[MTLK_WPS_STOP_ERROR][0] = wps_error_stop_err[0];
    LOG_LED("wps_errors[MTLK_WPS_STOP_ERROR] = %s",wps_errors[MTLK_WPS_STOP_ERROR]);
  }
}
...
{
  wps_status = val;
  LOG_LED("leds API: wps_status = %d",wps_status);
}
...
{
  cfg_mask = val;
  LOG_LED("leds API: cfg_mask = %d",cfg_mask);
}
...
{
  led_resolution = val;
  LOG_LED("leds API: led_resolution = %d",led_resolution);
}
...
int leds_ctrl_set_network_type(char* str)
{
  ...
  if (size <= sizeof(network_type)) {
    ...
    LOG_LED("leds API: network_type = %s",network_type);
    return MTLK_ERR_OK;
  }
...
int leds_ctrl_set_link_path(char* str)
{
  ...
  if (size <= sizeof(link_script_path)) {
    ...
    LOG_LED("leds API: link_script_path = %s",link_script_path);
    return MTLK_ERR_OK;
  }
...
int leds_ctrl_set_link_status_path(char* str)
{
  ...
  if (size <= sizeof(link_status_script_path)) {
    ...
    LOG_LED("leds API: link_status_script_path = %s",link_status_script_path);
    return MTLK_ERR_OK;
  }
...
char *leds_ctrl_get_bin_string(void)
{
  ...
  LOG_LED("index = %d",index);
  if (index < MTLK_CONF_BIN_PARAM_MAX) {
    LOG_LED("str_bin_array[index] = %s",str_bin_array[index]);
    return str_bin_array[index++];
  }
  LOG_LED("end of bin params");
  index = 0;
...
char *leds_ctrl_get_string(void)
{
  ...
  if (index < MTLK_CONF_PARAM_MAX) {
    LOG_LED("leds_ctrl_get_string: str_array entry = %s",str_array[index]);
...
void leds_ctrl_set_bin_param(const char *key, const int value)
{
  ...
  if (param) {
    *param = value;
    LOG_LED("value = %d, *param = %d",value,*param);
  }
  else
    ERROR("param was not found");
}
...
void leds_ctrl_set_param(const char *key, const char *value)
{
  ...
  if (param) {
    if (map_str_to_code(param, value)) {
      ERROR("str to code mapping error");
    }
    LOG_LED("test...value = %s, param = %s",value,param);
    set_error_array(key, value);
    LOG_LED("value = %s, param = %s",value,param);
  }
  else
    ERROR("param was not found");
}
...
{
  security_mode = val;
  LOG_LED("leds API: security_mode = %d",security_mode);
}

void leds_ctrl_set_reconf_security(int val)
{
  reconf_security = val;
  LOG_LED("leds API: reconf_security = %d",reconf_security);
}
...
dataex_report_error(int ioctl_rslt, char *pdata)
{
  ...
  case WE_GEN_DATAEX_DATABUF_TOO_SMALL:
    ...
    break;
  default:
    ERROR("Unknown data exchange error");
    break;
  }
}
...
leds_control(char * val, WE_GEN_DATAEX_STATUS* pstatus)
{
  ...
  if (!(cfg_mask & DHFLAG_NO_WRITE_LED) &&
    ...
    for (i = 0; i < MTLK_SECURITY_LEDS_INDEX_MIN; i++) {
      LOG_LED("i = %d, val = %s, frequency_band = %d, file = %s"
              ,i
              ,val
      ...
      if (memcmp(leds_config[i].file, "null", SAFE_SIZEOF("null")-1) == 0) {
        ...
        */
        ...
        if (val[0] == wlan_link_off[0] || pstatus->frequency_band == leds_config[i].index) {
          ...
          if (baseb_led[i] == CHAR_MAX) {
            ERROR("missmatch baseb_led param value");
            goto stop;
          }
          ...
          if (ioctl_rslt == -1) {
            WARNING("Can't write value to MAC\n");
            goto stop;
          }
        ...
        */
        LOG_LED("direct write to LED, pstatus->frequency_band=%d, leds_config[i].index=%d"
                 ,pstatus->frequency_band
                 ,leds_config[i].index);
        if (pstatus->frequency_band == leds_config[i].index) {
          ...
          if (fp == NULL)
          {
            WARNING("Error in open LED proc");
            goto stop;
            //return;
          }
          /*TODO: SAFE_LEN ??*/
          if (fwrite(val, 1, strlen(val), fp) < strlen(val))
            WARNING("Error write to LED proc");
    ...
    LOG_LED("masked or band was not set");
  }
stop:
  if (pdata)
    free(pdata);
...
leds_control_security(void)
{
  ...
  if (!(cfg_mask & DHFLAG_NO_WRITE_LED)) {
    ...
    for (i = MTLK_SECURITY_LEDS_INDEX_MIN; i < MTLK_SECURITY_LEDS_MAX; i++) {
      LOG_LED("i = %d, index = %d, file origin = %s, file new index = %s, val[%d] = %s"
              ,i
              ,leds_config[i].index
              ,leds_config[i].file
    ...
    LOG_LED("cfg_mask = 0x%x", cfg_mask);
}
...
parse_status(WE_GEN_DATAEX_DEVICE_STATUS* devst, WE_GEN_DATAEX_STATUS* pstatus, int sta)
{
  if ((devst->u32TxCount != u32TxCountLast[sta]) ||
    ...
    LOG_LED("sta=%d, data or scan, pstatus->scan_started=%d",sta,pstatus->scan_started);
    if (pstatus->scan_started)
      return wlan_link_scan[0];
    else {
      LOG_LED("Rx=%d, Tx=%d",devst->u32RxCount,devst->u32TxCount);
                      ...
                      ,int *do_leds)
{
  ...
  if (--reconf_security == 0) {
    if (load_security_cfg(params_conf_file) != MTLK_ERR_OK) {
      LOG_LED("error reading security param from %s",params_conf_file);
  ...
  if ((wps_status_mirror == wps_status_before) &&
    ...
    *do_leds = 0;
    ...
    LOG_LED("no event change, wps_status_mirror = %d, wps_status_before = %d",wps_status_mirror,wps_status_before);
  }
  else {
    /*Note:
      during WPS the security may change, therefore setting of security_leds_status may take place in leds*/
    *do_leds = 1;
    LOG_LED("event change: wps_status_mirror=%d (before=%d), security_on=%d (before=%d)"
             ,wps_status_mirror
             ,wps_status_before
  ...
  if (psecurity_status->security_on && psecurity_status->wep_enabled == 0)
    ...
    LOG_LED("wep_timeout, MTLK_SECURITY_LED_OFF");
  }

  switch (wps_status_mirror) {
    case MTLK_WPS_IDLE:
      LOG_LED("MTLK_WPS_IDLE");
    ...
    case MTLK_WPS_REGISTER:
      ...
      if (start_activate_timer) {
        ...
        LOG_LED("start timer in activate, time = %d",activate_start_time);
        LOG_LED("set led to code %s (%s)",wps_in_process, (security_leds_status[wps_activity_led]));
      }
       activate_current_time = time (NULL);
       LOG_LED("in activate condition, time left = %d"
      ...
      if (activate_current_time - activate_start_time >= wps_activated_timeout) {
        ...
        LOG_LED("activate T.O. report error");
        wps_status = MTLK_WPS_STOP_ERROR;
      }
    break;
    
    case MTLK_WPS_FINISH_OK:
      LOG_LED("MTLK_WPS_FINISH_OK");
      ...
      */
      if (psecurity_status->wep_enabled == 0 && psecurity_status->security_on) {
        ...
        if (wps_activity_led == security_led)
          ...
          strcpy(security_leds_status[wps_activity_led], security_off);
        LOG_LED("security_on, set led to code %s",security_leds_status[wps_activity_led]);
      }
      else {
        ...
        if (wep_start == 0) {
          LOG_LED("wep security mode or open, start timer");
          ...
          if (wep_current_time - wep_start_time >= security_wep_timeout) {
            LOG_LED("wep time elapse, change led status :-) ");
    ...
    case MTLK_WPS_STOP_PIN_ERROR:
      LOG_LED("MTLK_WPS_X (TIMEOUT/STOP_ERROR/STOP_OVERLAP)");
      if (error_start == 0) {
#if 0
        if (wps_status_mirror != wps_status_before) {
#endif
          ...
          */
          ...
          if (wps_activity_led != security_led)
            strcpy(security_leds_status[wps_activity_led], security_off);
          LOG_LED("MTLK_WPS_ERROR, entry = %i, code = %s",wps_status_mirror, wps_errors[wps_status_mirror]);
          strcpy(security_leds_status[wps_activity_led], wps_errors[wps_status_mirror]);
          LOG_LED("set security_leds_status to = %s",security_leds_status[wps_activity_led]);
          LOG_LED("time error_start_time = %ld",error_start_time);
          LOG_LED("wps_error_timeout = %d",wps_error_timeout);
#if 0
        }
#endif
      }
        ...
        if ((security_status_before != psecurity_status->security_on /*new event*/)&&
          ...
          /*if we have NEW security set event and error blink, clear error led.
            ...
            wps led N/A
          */
          LOG_LED("in error_start event, test if new connection established");
          error_start = 0;
          strcpy(security_leds_status[wps_error_led], security_off);
    ...
    /*some "illegal" codes are ok, for WPS scripts only*/
      LOG_LED("Error in wps_status_mirror, wps_status_mirror = %d"
               ,wps_status_mirror);
...
void leds_config_security_map(void) 
{
  ...
  wps_activity_led = leds_config[WPS_ACTIVITY_LED].index;
  wps_error_led = leds_config[WPS_ERROR_LED].index;
  LOG_LED("security_led = %d, wps_activity_led = %d, wps_error_led = %d"
...
int leds_parse_configs(void) 
{
  ...
  while ((i < MTLK_SECURITY_LEDS_MAX) && (open_count < MTLK_SECURITY_LEDS_MAX)) {
    ...
    if (!fp) {
      ERROR("error open %s file", leds_conf_file);
      return -1;
    }
    open_count++;
    while ( (fgets(line, MTLK_STRING_BUFFER_LINE_SIZE, fp) != NULL) &&
            (i < MTLK_SECURITY_LEDS_MAX) ) {
      LOG_LED("i = %d, search for %s, line  = %s",i, leds_config[i].entry, line);
      ...
      if (ptr) {
        ...
        LOG_LED("set file with %s, strlen(ptr) = %d"
                 ,leds_config[i].file, SAFE_LEN(ptr)-1);
#ifdef MTLK_DEBUG_CHARS
        for (j=0; j<13; j++)
          LOG0("char = 0x%x", leds_config[i].file[j]);
#endif
        ...
        if (strncmp("baseb", ptr, (SAFE_LEN(ptr)-MTLK_NEW_LINE_ADDITION-MTLK_LED_ADDITION)) == 0) {
          ...
          LOG_LED("baseb_led[%d] = 0x%x",leds_config[i].index,baseb_led[leds_config[i].index]);
        ...
        LOG_LED("string %s is not in this line",leds_config[i].entry);
      }
    }
    fclose(fp);
  }
  if (i == 0)
    ERROR("error, no string found in %s file", leds_conf_file);
...
wpa_state_complete(void)
{
  ...
  pid = fork();
  if (pid == 0) {
    LOG_LED("fork: write 0 to mtlk_data_link_status");
    char tmp[] = {(char)('w'), (char)(0x30), (char)0};
    if (execl(link_status_script_path, link_status_script_path, tmp, (char *)NULL) == -1)
      ERROR("error execute execl command for mtlk_data_link_status.sh");
  ...
  if (!fp) {
    LOG_LED("error open tmp_wpa_state_mirr file");
    return -1;
  }
  /*get the ptr to string*/
  ...
  if (ptr) {
    ...
    LOG_LED("string found, data link state, string = %s ",line);
    ret = 1;
  }
...
exec_script(char* script_ptr, char val)
{
  ...
  pid = fork();
  if (pid == 0) {
    LOG_LED("fork: exec %s script with value = %c",script_ptr,val);
    if (execl(script_ptr, script_ptr, tmp, (char *)NULL) == -1)
      ERROR("error execute execl command");
    exit(EVENT_DO_NOTHING);
  }
  res = waitpid(pid, NULL, 0);
  LOG_LED("out waitpid");
...
leds_thrdatamain (mtlk_handle_t context)
{
  ...
  for (;;) {
    ...
    */
    ...
    ioctl_rslt = driver_ioctl(MTLK_IOCTL_DATAEX, pdata, MTLK_IFCMD_DATALEN);
    if (ioctl_rslt == -1) {
      WARNING("Can't read security status; skipping LED update\n");
      goto stop;
    }

    presp = (WE_GEN_DATAEX_RESPONSE *) pdata;
    LOG_LED("presp->status = %d\n",presp->status);
    
    if (presp->status == WE_GEN_DATAEX_SUCCESS) {
      memcpy(&led_status, (WE_GEN_DATAEX_STATUS*)(presp+ 1),
                                     sizeof(WE_GEN_DATAEX_STATUS));

      LOG_LED("led data: led_status=%d, security_on=%d, scan_started=%d, frequency_band=%d, link_up = %d, wep_enabled=%d\n"
               ,presp->status
    ...
    */
    ...
    ioctl_rslt = driver_ioctl(MTLK_IOCTL_DATAEX, pdata, MTLK_IFCMD_DATALEN);
    if (ioctl_rslt == -1) {
      WARNING("Can't read connection statistics; skipping LED update");
      goto stop;
    }
    ...
    if (presp->status == WE_GEN_DATAEX_SUCCESS) {
      ...
      if (network_type[0] == MTLK_AP) {
        LOG_LED("network_type[0] = MTLK_AP, copy led_status.security_on from security_mode = %d\n",security_mode);
        led_status.security_on = (security_mode>=IW_ENCODE_ALG_TKIP)?1:0;
      }
      if (pqual->u8NumOfConnections > 0 || led_status.scan_started ||
          (network_type[0] == MTLK_AP && led_status.link_up == MTLK_LINK_UP)) {
        LOG_LED("u8NumOfConnections=%d, scan_started=%d, connection=%d, wps_status_mirror=%d \n"
                ,pqual->u8NumOfConnections
                ,led_status.scan_started
                ,connection
                ,wps_status_mirror);
        
        /*connection is up, test if need to indicate other application*/
        if ((link_script_path != NULL) && (connection == 0) && (pqual->u8NumOfConnections || led_status.link_up == MTLK_LINK_UP)) {
          ...
          if ((wps_status_mirror != MTLK_WPS_ACTIVATED) && (wps_status_mirror != MTLK_WPS_REGISTER)) {
            ...
            if (wpa_status == 1) {
              LOG_LED("wpa state completed, wps_status_mirror = %d",wps_status_mirror);
...
//            connection = 1;
        ...
        else if ((link_script_path != NULL) && (connection == 1) && (network_type[0] == 0x30 && pqual->u8NumOfConnections == 0)) {
          /*scan process. set flag to detect "statistics out of synch" (?change to support AP if it scan upon Init)*/
          was_in_scan = 1;
        }
        else if ((was_in_scan) && (link_script_path != NULL) && (connection == 1) && (pqual->u8NumOfConnections == 1)) {
          /*a patch:
            send disconnect to DHCP in case of statistic out of synch event*/
          LOG_LED("patch exec due to statistic mismatch");
          was_in_scan = 0;
          connection = 0;
        ...
        do_loop = pqual->u8NumOfConnections > led_status.scan_started ? pqual->u8NumOfConnections : led_status.scan_started;
        if (do_loop == 0 && network_type[0] == MTLK_AP)
          do_loop = 1;
        LOG_LED("do_loop = %d",do_loop);
        for (n = 0; n < do_loop; ++n) {
          WE_GEN_DATAEX_DEVICE_STATUS *devst = &pqual->sDeviceStatus[n];

          val[0] = parse_status(devst, &led_status, n);
          LOG_LED("ret: val = %s, val_to_write = %s, valbefore = %s\n",val,val_to_write,valbefore);
          ...
          */
          if (val[0] != valbefore[0]) {
            LOG_LED("val[0] != valbefore[0], n = %d, val = %s",n, val);
            if (n == 0)
        ...
        if (valbefore[0] != val_to_write[0]) {
          ...
          LOG_LED("call leds_control, val = %s",val);
          reset_led = 1;
          ...
          val[0] = wlan_link_off[0];
          valbefore[0] = val[0];
          LOG_LED("no connection reset led, val = %s",val);
