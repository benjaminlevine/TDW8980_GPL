...
//  char ifname[IFNAMSIZ];
  ...
  while (fgets(buffer, 1024, f)) {
    ...
    LOG1("Found network interface \"%s\"", itrfname);

    // Checking if interface supports wireless extensions...
    strncpy(req.ifr_name, itrfname, IFNAMSIZ);
    if (ioctl(gsocket, SIOCGIWNAME, &req)) {
      LOG1("Interface \"%s\" has no wireless extensions...", itrfname);
      continue;
    } else {
      LOG1("Interface \"%s\" has wireless extensions...", itrfname);
    }
...
driver_setup_connection(const char *itrfname)
{
  ...
  if (!found) {
    ERROR("Wireless interface \"%s\" not found", itrfname);
    return -1;
  }
...
driver_ioctl(int id, char *buf, size_t len)
{
  ...
  /*INFO("Calling ioctl %d (0x%X), data size %d", id, id, len);*/
  ...
  if (retval == -1) {
    ERROR("Ioctl call failed (code %d): %s", retval, strerror(errno));
    return -1;
  }

  /*INFO("Ioctl call: success (code %d)", retval);*/
  return retval;
}
