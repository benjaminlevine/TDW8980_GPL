...
_mtlk_cli_srv_hash_hash (const mtlk_cli_srv_key *key)
{
...
_mtlk_cli_srv_main_thread_proc (mtlk_handle_t context)
{
  ...
  while (1) {
    ...
    if (sd == INVALID_SOCKET) {
      WARNING("Can't accept socket (err=%d)", mtlk_socket_get_last_error());
      goto loop;
    }
    ...
    if (!peer) {
      ERROR("Can't allocate peer");
      goto loop;
    }

    res = _mtlk_cli_peer_init(peer);
    if (res != MTLK_ERR_OK) {
      ERROR("Can't initialize peer (err=%d)", res);
      goto loop;
    }
    peer_inited = 1;
...
_mtlk_cli_peert_hread_proc (mtlk_handle_t context)
{
  ...
  while (1) {
    ...
    if (res == MTLK_ERR_NOT_IN_USE) {
      ...
      ERROR("Can't get command string. Err#%d", res);
      goto cleanup;
    }
    ...
    if (res != MTLK_ERR_OK) {
      ERROR("Can't initiate cmd object. Err#%d", res);
      goto cleanup;
    }
    cmd_initialized = 1;

    if (mtlk_cli_srv_cmd_get_name(&cmd) == NULL) {
      LOG9("Empty cmd arrived. Skipping..."); /* looks like we got tail of previous command */
      res = MTLK_ERR_OK;
      goto cleanup;
    }
    ...
    if (res != MTLK_ERR_OK) {
      ERROR("Can't initiate rsp object. Err#%d", res);
      goto cleanup;
    }
    rsp_initialized = 1;
    ...
    if (res != MTLK_ERR_OK) {
      ERROR("Can't send responce. Err#%d", res);
      goto cleanup;
    }
    res = MTLK_ERR_OK;
