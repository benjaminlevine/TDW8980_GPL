...
nlink_wait_events(int socket_fd, int stop_fd)
{
  ...
  for(;;)
  {
    ...
    if (res == EINTR) {
      ...
      ERROR(DPR_PFX "select call failed with error: %s (%d)", strerror(errno), errno);
    ...
    WARNING(DPR_PFX "select returned without a reason");
  }
}
...
nlink_parse_msg(mtlk_nlink_socket_t* nlink_socket, struct mtlk_nl_msghdr *phdr)
{
  ...
  if (phdr->proto_ver != supported_protocol_version) {
    ERROR("Version mismatch: got %u, expected %u",
                  ...
                  mtlk_netlink_callback_t receive_callback, void* receive_callback_ctx)
{
  ...
  if (NULL == nlink_socket->sock) {
    ERROR(DPR_PFX "Generic netlink socket allocation failed");
    res = -1;
    goto end;
  }
  ...
  if (genl_connect(nlink_socket->sock) < 0) {
    ERROR("Connect to generic netlink controller failed");
    res = -1;
    goto err_dealloc;
  }
  ...
  if (nlink_socket->family < 0) {
    ERROR("Cannot get Generic Netlink family identifier.");
    res = -1;
    goto err_dealloc;
  }
  ...
  if (nl_socket_add_membership(nlink_socket->sock, irb_broadcast_group) < 0) {
    ERROR("Cannot add membership in %d group.", irb_broadcast_group);
    res = -1;
    goto err_dealloc;
  }
...
mtlk_nlink_receive_loop(mtlk_nlink_socket_t* nlink_socket, int stop_fd)
{
  ...
  for (;;) {
    ...
    if(EVENT_DATA_ARRIVED == wait_results) {
      ...
      ERROR(DPR_PFX "Wait for data failed (%d)", wait_results);
      return wait_results;
    }
  }
                  ...
                  mtlk_netlink_callback_t receive_callback, void* callback_ctx)
{
  ...
  if (-1 == nlink_socket->sock_fd) {
    res = -errno;
    ERROR(DPR_PFX "Failed to open netlink socket: %s (%d)", strerror(errno), errno);
    goto end;
  }
  ...
  if (-1 == bind(nlink_socket->sock_fd, (struct sockaddr *) &nlink_socket->src_addr, 
    ...
    res = -errno;
    ERROR(DPR_PFX "Failed to bind netlink socket: %s (%d)", strerror(errno), errno);
    goto err_dealloc;
  }
...
mtlk_nlink_receive_loop(mtlk_nlink_socket_t* nlink_socket, int stop_fd)
{
  ...
  if(NULL == nlh) {
    ERROR(DPR_PFX "Receive buffer allocation failed");
    return -ENOMEM;
  }
  ...
  for (;;) {
    ...
    if(EVENT_DATA_ARRIVED == wait_results) {
        ...
        case 0:
          res = 0;
          ERROR(DPR_PFX "Netlink peer performed shutdown");
          goto end; /* Peer performed shutdown */
        case -1: {
            res = -errno;
            ERROR(DPR_PFX "Receive message failed: %s (%d)", strerror(errno), errno);
            goto end;
          }
        default: {
