...
static int load_firmware_release(struct inode *i, struct file *f)
{
  ...
  if(data->count == 0)
    ELOG("File is missing...");
  else
    ILOG3(GID_COMPAT, "done (%d bytes)\n", data->count);
    ILOG1(GID_COMPAT, "Finished reading %s%s firmware file. Received %d bytes...\n",
...
static int load_firmware_thread(void *arg)
{
  ...
  argv[2] = temp;
  ILOG2(GID_COMPAT, "Loading firmware file %s%s... ", MTLK_FIRMWARE_PATH, (char *)arg);
  result = exec_usermodehelper(argv[0], argv, envp);
  if (result) {
    ILOG2(GID_COMPAT, "Failed [return = %d] to load firmware file %s%s", result, MTLK_FIRMWARE_PATH, (char *)arg);
  }
  ILOG2(GID_COMPAT, "Finished loading firmware");
  return result;
}
                      ...
                      const void *device)
{
  ...
  if(firmware_pentry == NULL) {
    ILOG2(GID_COMPAT, "Failed to create temporary proc-fs file");
    goto done;
  }
  ...
  if (pid < 0) {
    ILOG2(GID_COMPAT, "Failed to start thread...");
    goto done;
  }
  ...
  if (waitpid_result != pid)
    ELOG("sys_wait4(%d,...) failed, errno %d", pid, -waitpid_result);
  if(data.count == 0) {
    result = 1;
  }
  else {
