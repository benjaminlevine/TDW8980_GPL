        ...
        struct wpa_ie_data *data)
{
  ...
  if (hdr->elem_id != GENERIC_INFO_ELEM ||
    ...
    ELOG("malformed ie or unknown version");
    return -1;
  }
  ...
  if (left >= wpa_selector_len) {
    ...
    ELOG("ie length mismatch, %u too much", left);
    return -1;
  }
  ...
  if (left >= 2) {
    ...
    if (count == 0 || left < count * wpa_selector_len) {
      ELOG("ie count botch (pairwise), count %u left %u", count, left);
      return -1;
    }
    for (i = 0; i < count; i++) {
    ...
    ELOG("ie too short (for key mgmt)");
    return -1;
  }
  ...
  if (left >= 2) {
    ...
    left -= 2;
    if (count == 0 || left < count * wpa_selector_len) {
      ELOG("ie count botch (key mgmt), count %u left %u", count, left);
      return -1;
    }
    for (i = 0; i < count; i++) {
    ...
    ELOG("ie too short (for capabilities)");
    return -1;
  }
  ...
  if (left > 0)
    WLOG("ie has %u trailing bytes - ignored", left);
        ...
        struct wpa_ie_data *data)
{
  ...
  if (left >= rsn_selector_len) {
    ...
    ELOG("ie length mismatch, %u too much",
         left);
    return -1;
  }
  ...
  if (left >= 2) {
    ...
    if (left < data->num_pmkid * PMKID_LEN) {
      ELOG("PMKID underflow (num_pmkid=%d left=%d)",
           data->num_pmkid, left);
      data->num_pmkid = 0;
    } else {
      data->pmkid = pos;
