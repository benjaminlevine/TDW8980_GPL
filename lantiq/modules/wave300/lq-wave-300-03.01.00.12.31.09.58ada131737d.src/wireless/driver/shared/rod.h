...
typedef struct _reordering_queue
{
    ...
    rod_buff                 *buff[MAX_REORD_WINDOW];
...
{
    mtlk_osal_lock_init(&prod_queue->lock);
}
...
static __INLINE reordering_stats* 
mtlk_get_rod_stats(reordering_queue* prod_queue)
{
...
static __INLINE int
mtlk_is_used_rod_queue(reordering_queue* prod_queue)
{
...
void __MTLK_IFUNC
mtlk_rod_drop_packet(rod_buff *buff);

//OS-dependent function
//Should be implemented in rod_osdep.c
int __MTLK_IFUNC
mtlk_rod_detect_replay_or_sendup(rod_buff *skb, uint8 *rsn);
