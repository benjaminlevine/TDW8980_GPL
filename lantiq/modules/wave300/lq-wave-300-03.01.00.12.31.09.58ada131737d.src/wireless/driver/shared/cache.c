...
_cache_clone_country_ie(bss_data_t *bss)
{
  ...
  if (!country_ie) {
    WLOG("Cannot allocate memory for Country IE");
    goto FINISH;
  }
...
mtlk_cache_register_bss (scan_cache_t *cache, bss_data_t *bss)
{
  ...
  if (slot == NULL) {
    ...
    if (slot == NULL) {
      ILOG2(GID_CACHE, "Failed to register BSS %Y", bss->bssid);
      goto out;
    }
  ...
  ILOG2(GID_CACHE, "BSS %Y registered/updated", slot->bss.bssid);
...
mtlk_cache_get_next_bss (scan_cache_t *cache, bss_data_t *bss, int *freq, unsigned long *timestamp)
{
    ...
    while (cache->cur_entry && !res)
    {
        ...
        {
            ILOG3(GID_CACHE, "Expired cache entry removed: %Y (ch = %d)",
        ...
        {
            ILOG3(GID_CACHE, "Cache entry copied to user buffer: %Y (ch = %d)",
              cache->cur_entry->bss.bssid, cache->cur_entry->bss.channel);
            if (bss)
            {
...
******************************************************************************/
void __MTLK_IFUNC
mtlk_cache_remove_bss_by_bssid (scan_cache_t *cache, uint8 *bssid)
{
...
mtlk_cache_delete_current (scan_cache_t *cache)
{
    if (cache->cur_entry)
    {
        ...
        if (prev_list_entry)
        {
            ILOG4(GID_CACHE, "BSS removed from cache: %s", cache->cur_entry->bss.essid);
            mtlk_slist_remove_next(&cache->bss_list, prev_list_entry);
            cache->modified = 1;
            _cache_free_entry(cache->cur_entry);
        }
...
mtlk_cache_set_persistent(scan_cache_t *cache, const uint8 *bssid, BOOL is_persistent)
{
  ...
  while ((next = mtlk_slist_next(prev)) != NULL) {
    ...
    if (!memcmp(&entry->bss.bssid, bssid, IEEE_ADDR_LEN)) {
      ...
      ILOG2(GID_CACHE, "BSSID %Y, persistent=%i", bssid, is_persistent);
      break;
    }
...
BOOL __MTLK_IFUNC
mtlk_cache_is_wep_enabled(bss_data_t *bss)
{
  const uint16 WEP_mask = 0x0010;
  if (bss->capability & WEP_mask)
    return TRUE;
  else
    return FALSE;
}
