...
/* 
   ...
   printouts to DLevel N just define LOG2_ as LOG<N>.
*/
...
static void _mtlk_txmm_init_guard (mtlk_txmm_t* obj, mtlk_txmm_block_t* block)
{
...
static void _mtlk_txmm_check_guard (mtlk_txmm_t* obj, mtlk_txmm_block_t* block)
{
  uint8 *fguard = ((uint8 *)block->data.payload) - MTLK_TXMM_FGUARD_SIZE;
  uint8 *bguard = ((uint8 *)block->data.payload) + obj->cfg.max_payload_size;
...
static void _mtlk_txmm_block_history_commit (mtlk_txmm_block_t* block, mtlk_txmm_t* obj)
{
...
static void _mtlk_txmm_block_history_blocked_finalize_and_commit (mtlk_txmm_block_t* block, mtlk_txmm_t* obj, int wait_res)
{
...
static void _mtlk_txmm_history_print_unsafe (mtlk_txmm_t* obj)
{
    uint32 i = 0;
    for (i = 0; i < ARRAY_SIZE(obj->stats.history.entries); i++)
    {
        ...
        ELOG("MSG[%u]: id=0x%04X clbR=%u bwtR=%d", i, e->msg_id, e->clb_reason, e->blocked_res);
        for (j = 0; j < ARRAY_SIZE(e->ts); j++)
        {
            ELOG("    ts[%u] = %u", j, mtlk_osal_timestamp_to_ms(e->ts[j]));
        }
    }
}
...
static mtlk_txmm_block_t* _mtlk_txmm_get_buf(mtlk_txmm_t* obj, mtlk_txmm_block_e que_idx)
{
...
static void _mtlk_txmm_put_buf(mtlk_txmm_t* obj, mtlk_txmm_block_e que_idx, mtlk_txmm_block_t* buf)
{
  if (que_idx < SIZEOF(obj->ques))
  {
    mtlk_txmm_que_t* que = &obj->ques[que_idx];
...
static int _mtlk_txmm_del_buf_from_que(mtlk_txmm_t* obj, mtlk_txmm_block_e que_idx, mtlk_txmm_block_t* buf, int preserve_order)
{
  int res = -1;
  if (que_idx < SIZEOF(obj->ques))
  {
    ...
    {
      ELOG("deletion of invalid block "
           "from queue#%d (%p, %d, %d)", 
...
static __INLINE void _mtlk_txmm_free_buf(mtlk_txmm_t       *obj, 
...
typedef void (*mtlk_enum_que_f)(mtlk_txmm_t* obj, mtlk_txmm_block_t* buf);

static void _mtlk_txmm_enum_que(mtlk_txmm_t* obj, mtlk_txmm_block_e que_idx, mtlk_enum_que_f enum_f)
{
  mtlk_txmm_block_t* buf = NULL;
  if (que_idx < SIZEOF(obj->ques))
  {
...
static void _mtlk_txmm_err_print_que_block(mtlk_txmm_t* obj, mtlk_txmm_block_t* block)
{
  ELOG("\tMSG: id = 0x%04x, size = %d",
        block->data.id,
        block->data.payload_size);
}

static __INLINE void _mtlk_txmm_err_print_err_sent_que(mtlk_txmm_t* obj)
{
  ELOG("Unconfirmed messages:");
...
static void _mtlk_txmm_err_print_queues_unsafe(mtlk_txmm_t* obj)
{
    ELOG("Sent queue contents:");
    _mtlk_txmm_enum_que(obj, MTLK_TXMMB_SENT, _mtlk_txmm_err_print_que_block);
    ELOG("Pending queue contents:");
    _mtlk_txmm_enum_que(obj, MTLK_TXMMB_QUEUED, _mtlk_txmm_err_print_que_block);
    _mtlk_txmm_history_print_unsafe(obj);
}

static void _mtlk_txmm_send_block_prepare(mtlk_txmm_t* obj, mtlk_txmm_block_t* block, struct MSG_OBJ* pmsg)
{
...
static int _mtlk_txmm_pump(mtlk_txmm_t* obj, struct MSG_OBJ** _pmsg, unsigned* status)
{
  int             res  = MTLK_ERR_UNKNOWN;
  struct MSG_OBJ* pmsg = *_pmsg;


  ILOG4(GID_TXMM, "PUMP: MSG=%p nU=%d", 
  ...
  if (obj->ques[MTLK_TXMMB_QUEUED].total)
  {
    ...
    if (pmsg)
    {
      ...
      ILOG4(GID_TXMM, "PUMP: msg reusing...");
    ...
    {
      ILOG4(GID_TXMM, "PUMP: No msg to use...");
  ...
  {
    ILOG4(GID_TXMM, "PUMP: msg freeing...");
...
static BOOL _mtlk_txmm_call_usr_clb_locked (mtlk_txmm_t*           obj, 
                                            ...
                                            mtlk_txmm_clb_reason_e reason)
{
  ...
  default:
    ELOG("Unexpected callback action returned: %d", clb_action);
    MTLK_ASSERT(0);
    break;
  }
...
static uint32 _mtlk_txmm_on_timer_que(mtlk_txmm_t*           obj, 
                                      ...
                                      int                    preserve_order)
{
  ...
  for (;;)
  {
    ...
    ELOG("send: timeout (id=0x%04x). Res=%d (req=%u cfm=%u)",
          block->data.id,
          reason,
          obj->stats.nof_sent,
          obj->stats.nof_cfmed); 
...
{
  mtlk_txmm_t          *obj = HANDLE_T_PTR(mtlk_txmm_t, clb_usr_data);
...
static __INLINE int _mtlk_txmm_on_cfm(mtlk_txmm_t* obj, struct MSG_OBJ* pmsg)
{
  int                res   = MTLK_ERR_UNKNOWN; 
  mtlk_txmm_block_t* block = NULL;

  ILOG4(GID_TXMM, "OnCFM: MSG=%p nS=%d", 
  ...
  { /* there are sent messages */
    ...
    do
    {
      ...
      { /* Message found in sent queue */
        ILOG4(GID_TXMM, "OnCFM: msg found in SENT queue: IDX=%d", (int)i);
        block = que->ptr[i];
...
_mtlk_txmm_blocked_send_clb (mtlk_handle_t clb_usr_data, mtlk_txmm_data_t* data, mtlk_txmm_clb_reason_e reason)
{
  ...
  if (data->wait_data) /* set event on CFM enabled */
    mtlk_osal_event_set(&obj->bsend.evt);
...
int __MTLK_IFUNC mtlk_txmm_init(mtlk_txmm_t                *obj, 
...
int __MTLK_IFUNC mtlk_txmm_start(mtlk_txmm_t *obj)
{
...
static void _mtlk_txmm_attach_data(mtlk_txmm_t *obj, mtlk_txmm_msg_t *msg)
{
  ...
  block = _mtlk_txmm_get_buf(obj, MTLK_TXMMB_FREE);/* extract slot from free nodes pool */
  ...
  if (block) {
    ...
    ELOG("no free slot to get!");
    _mtlk_txmm_err_print_queues_unsafe(obj);
    msg->obj   = NULL;
    msg->data  = NULL;
  }
                               ...
                               uint32            timeout_ms)
{
  ...
  _mtlk_txmm_check_guard(obj, block);
  ...
  if (!obj->ques[MTLK_TXMMB_QUEUED].total && pmsg)
  { /* no nodes queued and message slot available => direct send message by block */
    ILOG4(GID_TXMM, "SEND: direct block sending: MSG=%p", pmsg);
    _mtlk_txmm_send_block_prepare(obj, block, pmsg);   /* prepare block, also put it to TXMMB_SENT queue */
    res = MTLK_ERR_OK;
  }
  else
  { /* node queueing */
    ILOG4(GID_TXMM, "SEND: block queued");
    _mtlk_txmm_put_buf(obj, MTLK_TXMMB_QUEUED, block);
   
    if (pmsg)
      /* message slot available */
      _mtlk_txmm_pump(obj, &pmsg, NULL); /* pump, i.e. prepare to send 1st block queued */
  ...
  if (pmsg)
    obj->api.msg_send(obj->api.usr_data, pmsg);
                                   ...
                                   uint32            timeout_ms)
{
  ...
  ASSERT(obj->flags & TXMM_FLAGS_ACTIVE); /* Nobody should call this function after the cleanup */
  
  mtlk_rmlock_acquire(&obj->bsend.rmlock);
  mtlk_osal_mutex_acquire(&obj->bsend.mutex);

  if ((obj->flags & TXMM_FLAGS_ACTIVE) == 0)
  { /* Cleanup issued. We must release all queued by mtlk_osal_mutex_acquire() threads */
    ILOG2(GID_TXMM, "send (b): cleaning up - ignored");
    goto FINISH;
  }

  mtlk_osal_event_reset(&obj->bsend.evt);

  data->wait_data = HANDLE_T(1); /* to enable event set on CFM */
  ...
  if (res != MTLK_ERR_OK)
  {
    ELOG("send (b): send failed. Err#%d", res);
    goto FINISH;
  }

  ILOG4(GID_TXMM, "send (b): wait event (%d ms)", (int)timeout_ms);
  res = mtlk_osal_event_wait(&obj->bsend.evt, timeout_ms);

  data->wait_data = HANDLE_T(0); /* to prevent event set on CFM */

  _mtlk_txmm_block_history_blocked_finalize_and_commit(_mtlk_txmm_block_by_data(data), obj, res);

  if (res != MTLK_ERR_OK)
  {
    ELOG("send (b): wait failed (id=0x%04x). Err#%d (req=%u cfm=%u)", 
          data->id,
          res, 
          obj->stats.nof_sent, 
          obj->stats.nof_cfmed);
  ...
  else if ((obj->flags & TXMM_FLAGS_ACTIVE) == 0) 
  {
    ILOG4(GID_TXMM, "send (b): cancelled by cleanup");
    goto FINISH;
  }
...
FINISH: 
  mtlk_osal_mutex_release(&obj->bsend.mutex);
  mtlk_rmlock_release(&obj->bsend.rmlock);
  return res;
}

static void _mtlk_txmm_free(mtlk_txmm_t* obj, mtlk_txmm_data_t* data)
{
  mtlk_txmm_block_t* block = _mtlk_txmm_block_by_data(data);
  ...
  case MTLK_TXMMB_FREE:
    ELOG("Second attempt to free msg (id=0x%04x)!", data->id);
    break;
...
int __MTLK_IFUNC mtlk_txmm_pump(mtlk_txmm_t* obj, struct MSG_OBJ* pmsg)
{
  int res;
  unsigned status;

  mtlk_osal_lock_acquire(&obj->lock);
  res = _mtlk_txmm_pump(obj, &pmsg, &status);
  mtlk_osal_lock_release(&obj->lock);
...
int __MTLK_IFUNC mtlk_txmm_on_cfm(mtlk_txmm_t* obj, struct MSG_OBJ* pmsg)
{
  int res = MTLK_ERR_UNKNOWN; 
...
void __MTLK_IFUNC mtlk_txmm_stop(mtlk_txmm_t *obj)
{
...
void __MTLK_IFUNC mtlk_txmm_cleanup(mtlk_txmm_t* obj)
{
  int i = 0;
  ...
  if (obj->flags & TXMM_FLAGS_ACTIVE) {
    ...
    ILOG2(GID_TXMM, "Wait until all the blocked sends are done...");
    i = mtlk_rmlock_wait(&obj->bsend.rmlock);/* Wait until all the blocked sends are done */
    ILOG2(GID_TXMM, "Wait finished (res = %d)", i);
...
void __MTLK_IFUNC mtlk_txmm_halt(mtlk_txmm_t* obj)
{
  obj->flags &= ~TXMM_FLAGS_ACTIVE;
}

BOOL __MTLK_IFUNC mtlk_txmm_is_running(mtlk_txmm_t* obj)
{
...
mtlk_txmm_msg_cleanup (mtlk_txmm_msg_t *msg)
{
  ...
  MTLK_CLEANUP_BEGIN(txmm_msg, MTLK_OBJ_PTR(msg))
    ...
    MTLK_CLEANUP_STEP(txmm_msg, TXMM_MSG_OBJPOOL, MTLK_OBJ_PTR(msg), 
                      mtlk_objpool_remove_object, (&g_objpool, &msg->obj_hdr, MTLK_TXMM_OBJ));
#endif
...
mtlk_txmm_msg_cancel (mtlk_txmm_msg_t *msg)
{
  ...
  if (obj) {
    ...
    if (msg->data) {
      ...
      _mtlk_txmm_free(msg->obj, msg->data);
