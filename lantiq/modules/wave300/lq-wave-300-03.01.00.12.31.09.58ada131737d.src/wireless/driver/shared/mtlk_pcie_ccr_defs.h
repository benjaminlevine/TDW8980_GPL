...
static __INLINE int
_mtlk_pcie_ccr_init(_mtlk_pcie_ccr_t *pcie_mem, mtlk_hw_bus_t *bus,
                    void* bar0, void* bar1) 
{
  MTLK_ASSERT(NULL != pcie_mem);
...
_mtlk_pcie_read_hw_timestamp(_mtlk_pcie_ccr_t *pcie_mem, uint32 *low, uint32 *high)
{
  ...
  __ccr_readl(pcie_mem->bus, &pcie_mem->pas->tsf_timer_low, *low);
  __ccr_readl(pcie_mem->bus, &pcie_mem->pas->tsf_timer_high, *high);
}
...
_mtlk_pcie_enable_interrupts(_mtlk_pcie_ccr_t *pcie_mem)
{
  ...
  mtlk_mmb_sync_isr(pcie_mem->bus->mmb, ___pcie_enable_interrupts, &ctx);

  if(ctx.call_interrupt_handler)
    mtlk_pci_handle_interrupt(pcie_mem->bus);
}
...
_mtlk_pcie_disable_interrupts(_mtlk_pcie_ccr_t *pcie_mem)
{
  ...
  mtlk_mmb_sync_isr(pcie_mem->bus->mmb, ___pcie_disable_interrupts, pcie_mem);
}
...
_mtlk_pcie_handle_interrupt(_mtlk_pcie_ccr_t *pcie_mem)
{
  ...
  if( pcie_mem->irqs_enabled )
  {
    return mtlk_pci_handle_interrupt(pcie_mem->bus);
  }
  else
  {
...
{
  MTLK_ASSERT(NULL != pcie_mem);
  __ccr_writel(pcie_mem->bus, 1, &pcie_mem->pas->HTEXT.door_bell);
}

static __INLINE  void
_mtlk_pcie_enable_xo(_mtlk_pcie_ccr_t *pcie_mem)
{
  MTLK_ASSERT(NULL != pcie_mem);
  /* No enable XO is needed on PCIE */
  MTLK_UNREFERENCED_PARAM(pcie_mem);
}

static __INLINE  void
_mtlk_pcie_disable_xo(_mtlk_pcie_ccr_t *pcie_mem)
{
  MTLK_ASSERT(NULL != pcie_mem);
  /* No disable XO is needed on PCIE */
  MTLK_UNREFERENCED_PARAM(pcie_mem);
}
...
static __INLINE  void
_mtlk_pcie_switch_clock(_mtlk_pcie_ccr_t *pcie_mem)
{
  MTLK_ASSERT(NULL != pcie_mem);
  /* Not needed on PCIE */
  MTLK_UNREFERENCED_PARAM(pcie_mem);
...
static __INLINE  void
_mtlk_pcie_unswitch_clock(_mtlk_pcie_ccr_t *pcie_mem)
{
  MTLK_ASSERT(NULL != pcie_mem);
  /* Not needed on PCIE */
  MTLK_UNREFERENCED_PARAM(pcie_mem);
...
static __INLINE  void
__pcie_open_secure_write_register(_mtlk_pcie_ccr_t *pcie_mem)
{
  MTLK_ASSERT(NULL != pcie_mem);

  __ccr_writel(pcie_mem->bus, 0xAAAA, 
               &pcie_mem->pas->RAB.secure_write_register);
  __ccr_writel(pcie_mem->bus, 0x5555, 
               &pcie_mem->pas->RAB.secure_write_register);
}
...
_mtlk_pcie_put_cpus_to_reset(_mtlk_pcie_ccr_t *pcie_mem)
{
  ...
  __pcie_open_secure_write_register(pcie_mem);
  __ccr_writel(pcie_mem->bus,
               new_cpu_boot_mode | (new_cpu_boot_mode << 8), 
...
static __INLINE  void
_mtlk_pcie_release_ucpu_reset(_mtlk_pcie_ccr_t *pcie_mem)
{
  uint32 new_cpus_state;
  ...
  new_cpus_state = G3_CPU_RAB_Override          |
                   pcie_mem->current_ucpu_state |
                   ( (pcie_mem->current_lcpu_state | G3_CPU_RAB_Override) << 8 );
...
static __INLINE  void
_mtlk_pcie_release_lcpu_reset(_mtlk_pcie_ccr_t *pcie_mem)
{
  uint32 new_cpus_state;
  ...
  new_cpus_state = ( (G3_CPU_RAB_Override |
                      pcie_mem->current_lcpu_state) << 8 ) |
                   (pcie_mem->current_ucpu_state | G3_CPU_RAB_Override);
...
_mtlk_pcie_check_bist(_mtlk_pcie_ccr_t *pcie_mem, uint32 *bist_result)
{
  ...
  MTLK_ASSERT(NULL != bist_result);
   ...
   __ccr_writel(pcie_mem->bus, G3_VAL_START_BIST, &pcie_mem->pas->HTEXT.start_bist);
   mtlk_osal_msleep(20);

  __ccr_readl(pcie_mem->bus, &pcie_mem->pas->HTEXT.bist_result, *bist_result);
...
__mtlk_pcie_wait_ack_assertion(_mtlk_pcie_ccr_t *pcie_mem, char* operation_id)
{
  ...
  {
    __ccr_readl(pcie_mem->bus, &pcie_mem->pas->HTEXT.phy_data, phy_status);
    if(phy_status & (1<<16)) return;
  }

  ILOG0(GID_CCR, "Ack assertion %s wait failed (phy_status = 0x%08x)\n", operation_id, phy_status);
  MTLK_ASSERT(FALSE);
}
...
__mtlk_pcie_wait_ack_deassertion(_mtlk_pcie_ccr_t *pcie_mem, char* operation_id)
{
  ...
  ILOG0(GID_CCR, "Ack deassertion %s wait failed (phy_status = 0x%08x)\n", operation_id, phy_status);
  MTLK_ASSERT(FALSE);
}
...
__mtlk_pcie_open_reg(_mtlk_pcie_ccr_t *pcie_mem, uint32 reg)
{
  ...
  __ccr_writel(pcie_mem->bus, (reg | (1<<16)), &pcie_mem->pas->HTEXT.phy_ctl);

  /* 3.Wait for CR_ack assertion */
  __mtlk_pcie_wait_ack_assertion(pcie_mem, "o1");

  /* 4.De-assert capture address bit */
  __ccr_writel(pcie_mem->bus, 0x00000000, &pcie_mem->pas->HTEXT.phy_ctl);
...
__mtlk_pcie_write_reg(_mtlk_pcie_ccr_t *pcie_mem, uint32 reg, uint16 val)
{
  ...
  __ccr_writel(pcie_mem->bus, ((1<<17)|val), &pcie_mem->pas->HTEXT.phy_ctl);
  ...
  /* 11.Set CR write bit */
  __ccr_writel(pcie_mem->bus, (1<<19), &pcie_mem->pas->HTEXT.phy_ctl);
...
__mtlk_pcie_read_reg(_mtlk_pcie_ccr_t *pcie_mem, uint32 reg) 
{
    ...
    __ccr_writel(pcie_mem->bus, (1<<18), &pcie_mem->pas->HTEXT.phy_ctl);

    /* 7.Wait for CR ack assertion */ 
    __mtlk_pcie_wait_ack_assertion(pcie_mem, "r3");

    /* 8.Read the <read data> from status bus */
    __ccr_readl(pcie_mem->bus, &pcie_mem->pas->HTEXT.phy_data, val);
...
{
  uint32 tx_ovr_before, tx_ovr_after, lvl_ovr_before, lvl_ovr_after;
  uint16 dataval;

  MTLK_ASSERT(NULL != pcie_mem);

  __mtlk_g3_release_ctl_from_reset_phase1(pcie_mem->bus, pcie_mem->pas);
  ...
  ILOG1(GID_CCR, "de-emphasis: TX input override modification: 0x%08x --> 0x%08x", tx_ovr_before, tx_ovr_after);
  ILOG1(GID_CCR, "de-emphasis: level override modification: 0x%08x --> 0x%08x", lvl_ovr_before, lvl_ovr_after);
#endif

  __mtlk_g3_release_ctl_from_reset_phase2(pcie_mem->bus, pcie_mem->pas);
}
