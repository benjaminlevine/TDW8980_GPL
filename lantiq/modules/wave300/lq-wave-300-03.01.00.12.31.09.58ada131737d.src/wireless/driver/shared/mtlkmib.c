                  ...
                  MIB_VALUE *uValue)
{
  ...
  if (!man_entry) {
    ELOG("Can't access MIB value due to lack of MAN_MSG");
...
#ifdef MTCFG_DEBUG
  {
    ...
    if (res == UMI_OK)
      ILOG2(GID_MIB, "Successfull %s, u16ObjectID = 0x%04x", s, u16ObjectID);
    else
      ILOG2(GID_MIB, "%s failed, u16ObjectID = 0x%04x, status = 0x%04x",
           s, u16ObjectID, res);
  }
#endif
                                   ...
                                   mtlk_aux_pm_related_params_t *result)
{
  ...
  man_entry = mtlk_txmm_msg_init_with_empty_data(&man_msg, txmm, NULL);
  if (!man_entry) {
    ELOG("No MM slot: failed to set pre-activation MIB");
    res = MTLK_ERR_NO_RESOURCES;
    goto FINISH;
  }
  ...
  if (!is_ht_net_mode(net_mode)) {
    ...
    result->u8SpectrumMode         = MIB_SPECTRUM_20M;
  }
  ...
  ILOG2(GID_MIB, "SENDING PREACTIVATION MIB:\n"
  ...
  if (res != MTLK_ERR_OK) {
    ELOG("Failed to set Pre-activation MIB (Err#%d)", res);
    goto FINISH;
  } else {  
  ...
  if (psSetMib->u16Status != HOST_TO_MAC16(UMI_OK)) {
    ELOG("Setting Pre-activation MIB status Failed");
    goto FINISH;
  }
