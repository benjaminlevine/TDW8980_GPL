...
#define MTLK_COC_RADIOOFF_POWER_MODE_TXNUM  (0)
#define MTLK_COC_RADIOOFF_POWER_MODE_RXNUM  (0)
...
{
  mtlk_coc_t    *coc_obj =
            mtlk_osal_mem_alloc(sizeof(mtlk_coc_t), MTLK_MEM_TAG_COC);

    if (NULL != coc_obj)
    {
        memset(coc_obj, 0, sizeof(mtlk_coc_t));

        _mtlk_coc_init(coc_obj, cfg);
    }
...
mtlk_coc_radiooff_power_mode_enable(mtlk_coc_t *coc_obj)
{
  ...
  res = _mtlk_coc_switch_mode(
            coc_obj,
            MTLK_COC_RADIOOFF_POWER_MODE_TXNUM,
            MTLK_COC_RADIOOFF_POWER_MODE_RXNUM);

  if (MTLK_ERR_OK == res)
  {
    coc_obj->current_antenna_cfg.num_tx_antennas = MTLK_COC_RADIOOFF_POWER_MODE_TXNUM;
    coc_obj->current_antenna_cfg.num_rx_antennas = MTLK_COC_RADIOOFF_POWER_MODE_RXNUM;
  }
...
_mtlk_coc_switch_mode(mtlk_coc_t *coc_obj, uint8 num_tx_antennas, uint8 num_rx_antennas)
{
  ...
  else
  {
    ...
    DUMP1(man_entry->payload, sizeof(UMI_CHANGE_POWER_STATE), "UMI_CHANGE_POWER_STATE dump:");
...
mtlk_coc_low_power_mode_get(mtlk_coc_t *coc_obj)
{
  ...
  {
    return COC_LOW_POWER_MODE;
  }
  else if ((MTLK_COC_RADIOOFF_POWER_MODE_TXNUM == coc_obj->current_antenna_cfg.num_tx_antennas) &&
           (MTLK_COC_RADIOOFF_POWER_MODE_RXNUM == coc_obj->current_antenna_cfg.num_rx_antennas))
  {
    return COC_RADIO_OFF_POWER_MODE;
  }

  return COC_HIGH_POWER_MODE;
}
