...
} __attribute__((aligned(1), packed)) IEEE_ADDR;
#endif
...
typedef struct _WE_GEN_DATAEX_CONNECTION_STATUS {
  uint8  u8GlobalNoise;
  uint8  u8ChannelLoad;
  uint8  u8NumOfConnections;
  uint8  u8padding0;
  uint16 u16NumDisconnections;
  uint16 u16padding1;
...
typedef struct _WLAN_WIRELESS_STATISTICS {
  uint64_t TransmittedFragmentCount;
  uint64_t MulticastTransmittedFrameCount;
  uint64_t FailedCount;
  uint64_t RetryCount;
  uint64_t MultipleRetryCount;
  uint64_t RTSSuccessCount;
  uint64_t RTSFailureCount;
  uint64_t ACKFailureCount;
  uint64_t FrameDuplicateCount;
  uint64_t ReceivedFragmentCount;
  uint64_t MulticastReceivedFrameCount;
  uint64_t FCSErrorCount;
} __attribute__((aligned(1), packed)) WLAN_WIRELESS_STATISTICS;
...
#define MTLK_WE_IOCTL_WMCE     (SIOCIWFIRSTPRIV + 3)
