...
# define MTLK_STEPS_INFO   LOG4

#define MTLK_STEPS_WARNING WARNING
#define MTLK_STEPS_ERROR   ERROR
...
#define _PERFORM_FAILURES_SIMULATION(error_val)                     \
  ...
  {                                                                 \
    MTLK_STEPS_WARNING(STEPS_PFX "*** Simulating failure ***");     \
    __res = (error_val);                                            \
...
#define _MTLK_FWD_STEPS_TRY(object, suffix)                         \
  for(;;) {                                                         \
...
#define _MTLK_FWD_STEP(exec_cond, module_name, step_name, object, success_op, iter_ctr, step_func,                        \
  ...
  {                                                                                                                       \
    ...
    ++g_CurrentStepNumber;                                                                                                \
    MTLK_STEPS_INFO(STEPS_PFX STEPS_TAB #module_name " " #suffix                                                          \
                        ", step " #step_name " iteration %d (#%d)...", iter_ctr, g_CurrentStepNumber);                    \
    ...
    if(exec_cond) {                                                                                                       \
      ...
      if (success_cond) {                                                                                                 \
        ...
        __res = (error_val);                                                                                              \
        MTLK_STEPS_ERROR(STEPS_PFX #module_name ":" #step_name " (step #%d) " #suffix " FAILED!", g_CurrentStepNumber);   \
        goto __ON_FWD_ERROR__;                                                                                            \
      }                                                                                                                   \
    } else {                                                                                                              \
      __res = MTLK_ERR_OK;                                                                                                \
      MTLK_STEPS_INFO(STEPS_PFX STEPS_TAB STEPS_TAB "Step SKIPPED because its precondition is false");                    \
...
#define _MTLK_FWD_STEP_VOID(exec_cond, module_name, step_name, object, post_run_op, iter_ctr, step_func,                  \
  ...
  {                                                                                                                       \
    ...
    __macro_mix_guard_fwd_##suffix = __macro_mix_guard_fwd_##suffix;                                                      \
    MTLK_STEPS_INFO(STEPS_PFX STEPS_TAB #module_name " " #suffix                                                               \
                        ", step " #step_name " iteration %d (VOID STEP)...", iter_ctr);                                   \
    if(exec_cond) {                                                                                                       \
      step_func func_args;                                                                                               \
...
__ON_FWD_ERROR__:                                                                                                                     \
  MTLK_STEPS_INFO(STEPS_PFX "Inner steps of " #module_name " un-" #suffix " started...");                                             \
  {                                                                                                                                   \
...
#define _MTLK_FWD_STEPS_RETURN(module_name, object, suffix, rollback_func, rollback_params)                             \
    ...
    MTLK_STEPS_ERROR(STEPS_PFX #module_name " " #suffix " FAILED! Initiating rollback...");                             \
...
#define _MTLK_BWD_STEPS_BEGIN(module_name, object, suffix)                                                                        \
  {                                                                                                                               \
    int __macro_mix_guard_bwd_##suffix = 0;                                                                                       \
    MTLK_STEPS_INFO(STEPS_PFX #module_name " un-" #suffix " started...");                                                         \
...
#define _MTLK_BWD_STEP(module_name, step_name, object, post_op, step_func, func_args, suffix)                                \
  {                                                                                                                          \
    ...
    if(!MTLK_U_BITS_GET(object __BFIELD_NAME(suffix, SKIPPED),                                                              \
      ...
      MTLK_STEPS_INFO(STEPS_PFX STEPS_TAB STEPS_TAB "Step was SKIPPED because precondition was false...");                   \
    }                                                                                                                        \
    if(MTLK_U_BITS_GET(object __BFIELD_NAME(suffix, MAIN),                                                                  \
                        __BIT_NUMBER(module_name, step_name, suffix), 1)) {                                                  \
      MTLK_STEPS_INFO(STEPS_PFX STEPS_TAB #module_name " un-" #suffix ", step " #step_name                                   \
                           " iteration %d done...", __exec_counter);                                                         \
...
#define _MTLK_BWD_STEPS_END(object, suffix)                                                                             \
    __macro_mix_guard_bwd_##suffix = __macro_mix_guard_bwd_##suffix;                                                    \
...
#define MTLK_INIT_TRY(module_name, object)                                                                              \
  MTLK_STEPS_INFO(STEPS_PFX #module_name " init started...");                                                           \
  _MTLK_FWD_STEPS_TRY(object, INIT);
...
#define MTLK_INIT_RETURN(module_name, object, cleanup_func, cleanup_params)                                             \
  MTLK_STEPS_INFO(STEPS_PFX #module_name " init finished");                                                             \
...
#define MTLK_CLEANUP_END(module_name, object)                                                                           \
  _MTLK_BWD_STEPS_END(object, INIT);                                                                                    \
  MTLK_STEPS_INFO(STEPS_PFX #module_name " cleanup finished");

/* Start/Stop macros */
#define MTLK_START_TRY(module_name, object)                                                                             \
  MTLK_STEPS_INFO(STEPS_PFX #module_name " startup started...");                                                        \
  _MTLK_FWD_STEPS_TRY(object, START);
...
#define MTLK_START_RETURN(module_name, object, stop_func, stop_params)                                                  \
  MTLK_STEPS_INFO(STEPS_PFX #module_name " start finished");                                                            \
...
#define MTLK_STOP_END(module_name, object)                                                                              \
  _MTLK_BWD_STEPS_END(object, START);                                                                                   \
  MTLK_STEPS_INFO(STEPS_PFX #module_name " stop finished");
