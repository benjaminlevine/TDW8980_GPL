...
mtlk_get_esnr_db (uint32 metric, uint8 mcs_feedback)
{
  ...
  LOG2("esnr=%u mcsf=%u rank=%u idx=%u beta=0x%08x arg=%f log=%f res=%u", 
       metric, mcs_feedback, rank, beta_index, s_beta_div_log2e[rank-1][beta_index], log_arg, log10(log_arg), res);
