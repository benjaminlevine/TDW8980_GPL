...
ifeq ($(DUMP),1)
  ...
  KERNEL_BUILD_DIR ?= $(BUILD_DIR_BASE)/linux-$(BOARD)$(if $(KERNEL_SUFFIX),_$(KERNEL_SUFFIX))$(if $(BUILD_SUFFIX),_$(BUILD_SUFFIX),$(if $(SUBTARGET),_$(SUBTARGET)))
...
define ModuleAutoLoad
	...
	if [ -n "$$$$$$$$modules" ]; then \
		mkdir -p $(2)/etc/modules.d; \
		mkdir -p $(2)/CONTROL; \
		echo "#!/bin/bash" > $(2)/CONTROL/postinst; \
