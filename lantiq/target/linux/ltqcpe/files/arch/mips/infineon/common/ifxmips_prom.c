...
strcpy(arcs_cmdline, "root=/dev/mtdblock2 rw rootfstype=squashfs ip=192.168.1.1:192.168.1.2::::eth0:on console=ttyS0,115200 \
ethaddr=AC:9A:96:F0:28:39 phym=64M mem=64M panic=1 \
mtdparts=ifx_sflash:128k(boot),1280k(kernel),6528k(rootfs),64k(config),64k(romfile),64k(rom),64k(radio) \
init=/sbin/init ethwan=");
