...
uint32_t ppa_check_is_special_session(PPA_BUF *ppa_buf, PPA_SESSION *p_session)
{
    ...
    /* added by Chen Zexian for dns, 2012.6.25 */
    const struct iphdr *iph = ip_hdr(ppa_buf);
    uint32_t proto = iph->protocol;
	...
	/* added by wangwenhao for massurl, 2012.5 */
#if defined(CONFIG_NF_CONNTRACK_MARK)
	if (p_session->mark & 0x40000000)
		return 1;
#endif
	/* end added */
...
int32_t ppa_lock_init(PPA_LOCK *p_lock)
{
    ...
    spin_lock_init((spinlock_t *)p_lock);
    return IFX_SUCCESS;
}
