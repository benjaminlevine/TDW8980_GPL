...
//#define PPA_SPEEDUP_TEST
    ...
    if ( (flags & PPA_F_BEFORE_NAT_TRANSFORM) )
    {
        ...
        ppa_lock_release(&g_session_list_lock);
         ...
         //create new session
        if ( ppa_add_session(ppa_buf, p_session, &p_item, flags) != IFX_SUCCESS )
        {
        ...
        p_item->priority = ppa_get_pkt_priority(ppa_buf);
            ...
            ppa_insert_session_item(p_item); //ppa_insert_session_item has its protection
        }
    ...
    else
    {
        ...
        // We have some concern about ppa_update_session since it needs to call net_if's stack API.
        // Note, so far there is no proection for net_if
        ...
        if ( (tmp_item.flags & SESSION_CAN_NOT_ACCEL) )
        {
        ...
        //Now copy tmp_item's value to PPA Session List
        ppa_lock_get(&g_session_list_lock);
        if ( __ppa_lookup_session(p_session, flags & PPA_F_SESSION_REPLY_DIR, &p_item) != IFX_PPA_SESSION_EXISTS )
        {
            int8_t strbuf[64];
            dbg("post routing point not exit after ppa_update_session: dst_ip = %s",
                ppa_get_pkt_ip_string(ppa_get_pkt_dst_ip(ppa_buf), ppa_is_pkt_ipv6(ppa_buf), strbuf));
            ppa_lock_release(&g_session_list_lock);
            return IFX_PPA_SESSION_NOT_ADDED;
        }
        ppa_memcpy( ((char *)p_item) + 4, ((char *)&tmp_item) + 4, sizeof(tmp_item)- 4 ); //skip first pointer next
...
int32_t ifx_ppa_set_session_inactivity(PPA_U_SESSION *p_session, int32_t timeout)
{
    ...
    ppa_lock_release(&g_session_list_lock);
    ppa_set_polling_timer(timeout, 0); //internally there is lock in ppa_set_polling_timer
    return IFX_SUCCESS;
}
