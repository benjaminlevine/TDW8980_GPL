...
static int ppe_open(struct atm_vcc *vcc)
{
    ...
    *WTX_QUEUE_CONFIG(sw_tx_queue) = wtx_queue_config;
	...
	set_bit(ATM_VF_ADDR, &vcc->flags);	/* wzy: claim address, "atmarp -a" will use this flag; 22Aug12 */
...
static void ppe_close(struct atm_vcc *vcc)
{
	...
	clear_bit(ATM_VF_ADDR, &vcc->flags);	/* wzy: clear address flag, 22Aug12	*/
...
PPE_CLOSE_EXIT:
	clear_bit(ATM_VF_READY, &vcc->flags);	/* wzy: clear ready flag, 22Aug12	*/
    up(&g_sem);
}
...
static void eth_setup(struct net_device *dev)
{
	...
	/* added by yangxv for get eth mac form flash, 2012.04 */
	{
		#define MAC_ADDR_SIZE 6
		extern int read_mac_from_flash(unsigned char *pMac);
		unsigned char mac_addr[MAC_ADDR_SIZE] = {0};
		unsigned int mac_without_oui = 0;
		unsigned char *ul = NULL;

		ul = (unsigned char *)&mac_without_oui;

		read_mac_from_flash(mac_addr);

#if defined(__BIG_ENDIAN)
		memcpy(ul + 1,  mac_addr + 3, MAC_ADDR_SIZE - 3);

		mac_without_oui = mac_without_oui + port;

		memcpy(mac_addr + 3, ul + 1, MAC_ADDR_SIZE - 3);
...
extern void tl_gpio_blinking(int led_index);

static INLINE void adsl_led_flash(void)
{
	/* added by yangxv for internet LED, 2012.4 */
	tl_gpio_blinking(TL_LED_INTERNET);
	/* end added */
...
#if 0 /* Wu Zhiqin, 2012-05-28 not start here, we will start 0--6 port when systen have setup */
    for ( i = 0; i < 7; i++ ) {
