...
void F_REN(print_trb_offsets)(struct xhci_hcd *xhci, union xhci_trb *trb)
{
	...
	for (i = 0; i < 4; ++i)
		xhci_dbg(xhci, "Offset 0x%x = 0x%08x[U:0x%08x]\n",
				i*4, DescAdj32(trb->generic.field[i]),trb->generic.field[i]);
}
...
void F_REN(debug_trb)(struct xhci_hcd *xhci, union xhci_trb *trb)
{
	...
	u64	address;
	#endif
	u32	type = DescAdj32(trb->link.control) & TRB_TYPE_BITMASK;

	switch (type) {
	case TRB_TYPE(TRB_LINK):
		xhci_dbg(xhci, "Link TRB:\n");
		F_REN(print_trb_offsets)(xhci, trb);
		...
		xhci_dbg(xhci, "Interrupter target = 0x%x\n",
				GET_INTR_TARGET(DescAdj32(trb->link.intr_target)));
		xhci_dbg(xhci, "Cycle bit = %u\n",
				(unsigned int) (DescAdj32(trb->link.control) & TRB_CYCLE));
		xhci_dbg(xhci, "Toggle cycle bit = %u\n",
				(unsigned int) (DescAdj32(trb->link.control) & LINK_TOGGLE));
		xhci_dbg(xhci, "No Snoop bit = %u\n",
				(unsigned int) (DescAdj32(trb->link.control) & TRB_NO_SNOOP));
		break;
	...
	case TRB_TYPE(TRB_COMPLETION):
		#if defined(__XHCI_DESC_SWAP__) || defined(__XHCI_DESC_SWAP64__)
		address = DescAdj32(trb->event_cmd.cmd_trb);
		...
		xhci_dbg(xhci, "Completion status = %u\n",
				(unsigned int) GET_COMP_CODE(DescAdj32(trb->event_cmd.status)));
		xhci_dbg(xhci, "Flags = 0x%x\n", (unsigned int) DescAdj32(trb->event_cmd.flags));
		break;
	default:
...
void F_REN(decode_trb)(u32 addr,union xhci_trb *trb){
	if(!trb->link.segment_ptr && !trb->link.intr_target && !trb->link.control)
		return;
	switch((DescAdj32(trb->link.control) & 0x0000FC00) >>10)
	{
		case 32:
			printk(KERN_INFO "      @%08x TransferEvent %08x", addr, DescAdj32(trb->link.segment_ptr));
			printk(KERN_INFO " CompletionCode:%02x Length:%06x",
					(DescAdj32(trb->link.intr_target) & 0xFF000000) >>24,
					(DescAdj32(trb->link.intr_target) & 0x00FFFFFF));
			printk(KERN_INFO " Slot:%02x EPId:%02x ED:%d C:%d\n",
					(DescAdj32(trb->link.control) & 0xFF000000) >>24,
					(DescAdj32(trb->link.control) & 0x001F0000) >>16,
					(DescAdj32(trb->link.control) & 0x00000004) >>2 ,
					(DescAdj32(trb->link.control) & 0x00000001) >>0 );
			break;
		case 33:
			printk(KERN_INFO "      @%08x CommandCompleteEvent %08x", addr, DescAdj32(trb->link.segment_ptr));
			printk(KERN_INFO " CompletionCode:%02x",
					(DescAdj32(trb->link.intr_target) & 0xFF000000) >>24);
			printk(KERN_INFO " Slot:%02x VFId:%02x C:%d\n",
					(DescAdj32(trb->link.control) & 0xFF000000) >>24,
					(DescAdj32(trb->link.control) & 0x00FF0000) >>16,
					(DescAdj32(trb->link.control) & 0x00000001) >>0 );
			break;
		case 34:
			printk(KERN_INFO "      @%08x PortStatusChgEvent %08x", addr, DescAdj32(trb->link.segment_ptr));
			printk(KERN_INFO " CompletionCode:%02x",
					(DescAdj32(trb->link.intr_target) & 0xFF000000) >>24);
...
void F_REN(dbg_ring_ptrs)(struct xhci_hcd *xhci, struct xhci_ring *ring)
{
	xhci_dbg(xhci, "Ring deq = %p (virt), 0x%llx (dma)\n",
			ring->dequeue,
			(unsigned long long)F_REN(trb_virt_to_dma)(ring->deq_seg,
							    ring->dequeue));
	xhci_dbg(xhci, "Ring deq updated %u times\n",
			ring->deq_updates);
	xhci_dbg(xhci, "Ring enq = %p (virt), 0x%llx (dma)\n",
			ring->enqueue,
			(unsigned long long)F_REN(trb_virt_to_dma)(ring->enq_seg,
							    ring->enqueue));
...
void F_REN(debug_ring)(struct xhci_hcd *xhci, struct xhci_ring *ring)
{
	...
	F_REN(debug_segment)(xhci, first_seg);
...
void F_REN(dbg_erst)(struct xhci_hcd *xhci, struct xhci_erst *erst)
{
	...
	for (i = 0; i < erst->num_entries; ++i) {
		...
		xhci_dbg(xhci, "@%08x %08x %08x %08x %08x\n",
				(unsigned int) addr,
				(unsigned int) DescAdj32(entry->seg_addr),
				(unsigned int) DescAdj32(entry->seg_addr_reserved),
				(unsigned int) DescAdj32(entry->seg_size),
				(unsigned int) DescAdj32(entry->rsvd));
		#else
		xhci_dbg(xhci, "@%08x %08x %08x %08x %08x\n",
				(unsigned int) addr,
...
static void dbg_rsvd64(struct xhci_hcd *xhci, u64 *ctx, dma_addr_t dma)
{
	...
	for (i = 0; i < 4; ++i) {
		xhci_dbg(xhci, "@%p (virt) @%08llx "
			 ...
			 DescAdj32((u32)(ctx[4 + i])),(u32)ctx[4 + i], i);
		dma += 8;
	}
}

void F_REN(dbg_slot_ctx)(struct xhci_hcd *xhci, struct xhci_container_ctx *ctx)
{
	/* Fields are 32 bits wide, DMA addresses are in bytes */
//	int field_size = 32 / 8;
//	int i;
	struct xhci_slot_ctx *slot_ctx = F_REN(get_slot_ctx)(xhci, ctx);
	...
	xhci_dbg(xhci, "   Route String:%05X\n",(DescAdj32(slot_ctx->dev_info)&0x000FFFFF)>>0);
	xhci_dbg(xhci, "   Speed ID:%d\n",(DescAdj32(slot_ctx->dev_info)&0x00F00000)>>20);
	xhci_dbg(xhci, "   Multi TT:%d\n",(DescAdj32(slot_ctx->dev_info)&0x02000000)>>25);
	xhci_dbg(xhci, "   Hub:%d\n",(DescAdj32(slot_ctx->dev_info)&0x04000000)>>26);
	xhci_dbg(xhci, "   Context Entries:%d\n",(DescAdj32(slot_ctx->dev_info)&0xF8000000)>>27);

	xhci_dbg(xhci, "   Max Exit Latency:%04X\n",(DescAdj32(slot_ctx->dev_info2)&0x0000FFFF)>>0);
	xhci_dbg(xhci, "   RootHub Port Number:%02X\n",(DescAdj32(slot_ctx->dev_info2)&0x00FF0000)>>16);
	xhci_dbg(xhci, "   Number of Ports:%02X\n",(DescAdj32(slot_ctx->dev_info2)&0xFF000000)>>24);

	xhci_dbg(xhci, "   TT Hub Slot ID:%02X\n",(DescAdj32(slot_ctx->tt_info)&0x000000FF)>>0);
	xhci_dbg(xhci, "   TT Port Number:%0X\n",(DescAdj32(slot_ctx->tt_info)&0x0000FF00)>>8);
	xhci_dbg(xhci, "   TT Think Time:%d\n",(DescAdj32(slot_ctx->tt_info)&0x00030000)>>16);
	xhci_dbg(xhci, "   Interrupt Target:%03X\n",(DescAdj32(slot_ctx->tt_info)&0xFFC00000)>>22);

	xhci_dbg(xhci, "   Device Addr:%02X\n",(DescAdj32(slot_ctx->dev_state)&0x000000FF)>>0);
	switch((DescAdj32(slot_ctx->dev_state)&0xF8000000)>>27)
	{
		...
		default: xhci_dbg(xhci, "   Slot State:  Rsv %d\n",(DescAdj32(slot_ctx->dev_state)&0xF8000000)>>27); break;
	}
...
void F_REN(dbg_ep_ctx)(struct xhci_hcd *xhci,
...
//	int j;
	...
	for (i = 0; i <=last_ep_ctx; ++i) {
		struct xhci_ep_ctx *ep_ctx = F_REN(get_ep_ctx)(xhci, ctx, i);
		dma_addr_t dma = ctx->dma +
		...
		switch(DescAdj32(ep_ctx->ep_info)&0x00000007)
		{
		...
		switch((DescAdj32(ep_ctx->ep_info)&0x00000300)>>8)
		{
			case 0:
			case 1:
			case 2:
				xhci_dbg(xhci, "   Multi (Burst) :%d\n",(DescAdj32(ep_ctx->ep_info)&0x00000300)>>8); break;
			case 3:
				xhci_dbg(xhci, "   Multi (Burst) :%d (Unsupported)\n",(DescAdj32(ep_ctx->ep_info)&0x00000300)>>8); break;
		}
		xhci_dbg(xhci, "   MaxPStream :%d\n",(DescAdj32(ep_ctx->ep_info)&0x00007C00)>>10);
		xhci_dbg(xhci, "   LinearStreamArray :%d\n",(DescAdj32(ep_ctx->ep_info)&0x00008000)>>15);
		xhci_dbg(xhci, "   Interval :%d\n",(DescAdj32(ep_ctx->ep_info)&0x00FF0000)>>16);

		xhci_dbg(xhci, "   ErrorCount :%d\n",(DescAdj32(ep_ctx->ep_info2)&0x00000006)>>1);
		switch((DescAdj32(ep_ctx->ep_info2)&0x00000038)>>3)
		{
...
void F_REN(dbg_ctx)(struct xhci_hcd *xhci,
		  struct xhci_container_ctx *ctx,
		  unsigned int last_ep)
{
	int i;
	...
	if (ctx->type == XHCI_CTX_TYPE_INPUT) {
		struct xhci_input_control_ctx *ctrl_ctx =
			F_REN(get_input_control_ctx)(xhci, ctx);
		xhci_dbg(xhci, "@%p (virt) @%08llx (dma) %08x - drop flags\n",
			 &ctrl_ctx->drop_flags, (unsigned long long)dma,
			 DescAdj32(ctrl_ctx->drop_flags));
		dma += field_size;
		xhci_dbg(xhci, "@%p (virt) @%08llx (dma) %08x - add flags\n",
			 &ctrl_ctx->add_flags, (unsigned long long)dma,
			 DescAdj32(ctrl_ctx->add_flags));
		dma += field_size;
		for (i = 0; i < 6; ++i) {
			xhci_dbg(xhci, "@%p (virt) @%08llx (dma) %08x - rsvd2[%d]\n",
				 &ctrl_ctx->rsvd2[i], (unsigned long long)dma,
				 DescAdj32(ctrl_ctx->rsvd2[i]), i);
			dma += field_size;
		}
	...
	slot_ctx = F_REN(get_slot_ctx)(xhci, ctx);
	F_REN(dbg_slot_ctx)(xhci, ctx);
	F_REN(dbg_ep_ctx)(xhci, ctx, last_ep);
}
