 ...
 *
 * history 	\arg	2012-6 modified by tp-link hanjiayan.
 *
 */
...
//Dump conntrack infomation
static void dump_conntrack(struct nf_conn *ct, enum ip_conntrack_info ctinfo)
{
	enum ip_conntrack_dir dir = CTINFO2DIR(ctinfo);
	printk("----------------------------------------\n");
	printk("Conntrack Info: Dir = %d\n", dir);
...
static void dump_expect_list(struct nf_conn *ct)
{
	...
	printk("\tSIP_EXPECT_SIGNALLING count: %d, max %d, timeout %d\n", 
		help->expecting[SIP_EXPECT_SIGNALLING], 
		help->helper->expect_policy[SIP_EXPECT_SIGNALLING].max_expected,
		help->helper->expect_policy[SIP_EXPECT_SIGNALLING].timeout);

	printk("\tSIP_EXPECT_AUDIO      count: %d, max %d, timeout %d\n\n", 
		help->expecting[SIP_EXPECT_AUDIO], 
		help->helper->expect_policy[SIP_EXPECT_AUDIO].max_expected,
		help->helper->expect_policy[SIP_EXPECT_AUDIO].timeout);
	...
	/*Dump expect list*/
	i = 0;
	h = nf_ct_expect_dst_hash(&ct->tuplehash[1].tuple);
	hlist_for_each_entry_rcu(exp, n, &net->ct.expect_hash[h], hnode)
	{ 
		i++;
		printk("\t----\t%d:  timer %ld src=%u.%u.%u.%u:%u, dst=%u.%u.%u.%u:%u"
			", local=%u.%u.%u.%u:%u\n",
			i, 
			(long)(exp->timeout.expires - jiffies)/HZ, 
			NIPQUAD(exp->tuple.src.u3.ip), exp->tuple.src.u.udp.port,
					  ...
					  unsigned int expires)
{
	...
	SIP_ALG_DBG("Looking for expectation src=0.0.0.0:0 dst=%u.%u.%u.%u:%u",
	...
	if(NULL == exp)
	{
		SIP_ALG_DBG("SIGNALLING expectation not find, add time error!");
		goto out;
	}
	...
	if(del_timer(&exp->timeout))
	{
		...
		SIP_ALG_DBG("SIGNALLING expectation add time (%d) ok!", 
			(expires + SIP_EXPECT_SIGNALLING_TIMEOUT));
		SIP_ALG_DBG("----Timer %ld, src=%u.%u.%u.%u:%u, dst=%u.%u.%u.%u:%u"
			", local=%u.%u.%u.%u:%u\n",
			(long)(exp->timeout.expires - jiffies)/HZ, 
			NIPQUAD(exp->tuple.src.u3.ip), exp->tuple.src.u.udp.port,
			NIPQUAD(exp->tuple.dst.u3.ip), exp->tuple.dst.u.udp.port,
	...
	{
		SIP_ALG_DBG("SIGNALLING expectation delete timer error! add time (%d) error!!", expires);
	}
					 ...
					 __be16 port)
{
	...
	if(NULL == exp)
	{
		SIP_ALG_DBG("SIGNALLING expectation not find, delete time error!");
		goto out;
	}

	SIP_ALG_DBG("Find exp: (%s)----Timer %ld, src=%u.%u.%u.%u:%u, dst=%u.%u.%u.%u:%u"
			", local=%u.%u.%u.%u:%u\n",
			(exp->class ? "SIP_EXPECT_AUDIO" : "SIP_EXPECT_SIGNALLING"),
			(long)(exp->timeout.expires - jiffies)/HZ, 
			NIPQUAD(exp->tuple.src.u3.ip), exp->tuple.src.u.udp.port,
			NIPQUAD(exp->tuple.dst.u3.ip), exp->tuple.dst.u.udp.port,
			NIPQUAD(exp->saved_ip), exp->saved_proto.udp.port);
	...
	if (del_timer(&exp->timeout))
	{
		...
		SIP_ALG_DBG("SIGNALLING flush expectation ok!");
	}
	else
	{
		SIP_ALG_DBG("SIGNALLING flush expectation, delete timer error!");
	}
out:
	spin_unlock_bh(&nf_conntrack_lock);
...
/* 
 * brief	hanjiayan@5Sep2012, Add For local directly Response.
 */
		       ...
		       unsigned int cseq)
{
	...
	SIP_ALG_DBG("Build SIP response expectation");

	if (ct->tuplehash[dir].tuple.src.u3.ip != ct->tuplehash[!dir].tuple.dst.u3.ip)
	{
		SIP_ALG_DBG("Local out package, don't build invite response expectation!");
		return NF_ACCEPT;
	}
	else
	{
		SIP_ALG_DBG("Remote in package, build invite response expectation!");
	}
	...
	{	
		SIP_ALG_DBG("Inter call, Don't build invite response expectation!");
		return 0;
	}
	...
	SIP_ALG_DBG("Build SIP response expectation :src=%u.%u.%u.%u:%u,"
			" dst=%u.%u.%u.%u:%u",		
				 ...
				 unsigned int mediaoff, unsigned int medialen)
{
	...
	if (ct->tuplehash[dir].tuple.src.u3.ip == ct->tuplehash[!dir].tuple.dst.u3.ip)
	{
		SIP_ALG_DBG("Not local out SDP, don't build RTP expectation!");
		return NF_ACCEPT;
	}
	else
	{
				   ...
				   unsigned int cseq, unsigned int code)
{
	if ((code >= 100 && code <= 199) ||
		(code >= 200 && code <= 299))	
	{
		/*Start of hanjiayan@5Sep2012, Add For local directly Response.*/
		set_expected_invite_response(skb, dptr, datalen, cseq);
		/*End of hanjiayan@5Sep2012, Add For local directly Response.*/
...
static int process_cancel_request(struct sk_buff *skb,
			       const char **dptr, unsigned int *datalen,
			       unsigned int cseq)
{	
	return NF_ACCEPT;
				    ...
				    unsigned int cseq)
{
	...
	SIP_ALG_DBG("Register contact : %u.%u.%u.%u:%u", NIPQUAD(daddr.ip), port);

	/* We don't support third-party registrations */
	if (!nf_inet_addr_cmp(&ct->tuplehash[dir].tuple.src.u3, &daddr))
	{
		SIP_ALG_DBG("We don't support third-party registrations : (src=%u.%u.%u.%u:%u)",
				NIPQUAD(ct->tuplehash[dir].tuple.src.u3.ip), port);
		return NF_ACCEPT;
	}
	...
	if (expires == 0)
	{
		SIP_ALG_DBG("SIP unregister request!");
		ret = NF_ACCEPT;
		return ret;
	}

	/*If register is direct signalling, Don't need expectation*/
	...
	{	
		...
		/*Dump expectation*/
		SIP_ALG_DBG("Looking for signalling expectation :src=%u.%u.%u.%u:%u,"
			" dst=%u.%u.%u.%u:%u",		
			NIPQUAD(exp->tuple.src.u3.ip), ntohs(exp->tuple.src.u.udp.port), 	
			NIPQUAD(exp->tuple.dst.u3.ip), ntohs(exp->tuple.dst.u.udp.port));
		...
		{
			SIP_ALG_DBG("SIGNALLING expectation request, Expires (%ld), Don't need refresh!",
				(long)(exp->timeout.expires - jiffies) /HZ);
			break;
		}
		...
		if (del_timer(&exp->timeout))
		{
			SIP_ALG_DBG("SIGNALLING expectation request, Expires (%ld), Refresh timer (%d)", 
				(long)(exp->timeout.expires - jiffies) /HZ,
	...
	if(!found)
	{
		SIP_ALG_DBG("SIGNALLING expectation not find, build it!");
...
static const struct sip_handler sip_handlers[] = {
	...
	SIP_HANDLER("CANCEL", process_cancel_request, NULL),
...
static const struct nf_conntrack_expect_policy sip_exp_policy[SIP_EXPECT_MAX + 1] = {
	...
	[SIP_EXPECT_AUDIO] = {
		...
		.timeout	= SIP_EXPECT_MEIDA_TIMEOUT,
	},
};
