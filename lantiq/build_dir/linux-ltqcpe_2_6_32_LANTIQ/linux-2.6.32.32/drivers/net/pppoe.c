...
static int pppoe_release(struct socket *sock)
{
	...
	/* release socket的时候应当把它使用的dev的refcnt --，否则在删除接口的时候会被锁在内核里。
	 * 这里的判断条件中漏掉了PPPOX_ZOMBIE。参考ralink、realtek的内核，此处修改为判断dev指针是否为空。
	 * 因为执行完refcnt--之后，该dev指针会被置为NULL。  by Yin Jun, 2012-09-14.
	 */
