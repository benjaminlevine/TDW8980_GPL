 ...
 * brief	Print tuple
 */
#define SIP_ALG_DUMP_TPL(tp)	\
SIP_ALG_DBG("------ src=%u.%u.%u.%u:%u  dst=%u.%u.%u.%u:%u\n",		\
	NIPQUAD((tp)->src.u3.ip), ntohs((tp)->src.u.udp.port), 	\
	NIPQUAD((tp)->dst.u3.ip), ntohs((tp)->dst.u.udp.port))

#define SIP_PORT	5060
#define SIP_TIMEOUT	3600

/* 
 * brief	SIP 端口期待连接超时时间
 */
#define SIP_EXPECT_SIGNALLING_TIMEOUT 	10
/* 
 * brief	RTP 端口期待连接超时时间
 */
#define SIP_EXPECT_MEIDA_TIMEOUT 	3*60
