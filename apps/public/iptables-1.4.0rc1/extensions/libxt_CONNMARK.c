...
"  --next [CONTINUE|ACCEPT|DROP|RETURN]"
"\n",
IPTABLES_VERSION);
}
               ...
               const void *entry, struct xt_entry_target **target)
{
	...
	case '5':
		if (!*flags)
		exit_error(PARAMETER_PROBLEM,
			           "CONNMARK target: Can't specify --next without a operation");
...
static void CONNMARK_init(struct xt_entry_match *m)
{
	struct xt_connmark_target_info *info = (void *)m->data;
