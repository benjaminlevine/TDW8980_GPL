...
uint8_t *get_option(struct dhcpMessage *packet, int code)
{
	...
	i = 0;
	/* 308; options buffer length should be exactly what it is, added by tiger 20090922 */
	length = sizeof(packet->options);

	while (!done) {
		if (i >= length) {
			LOG_DEBUG("Bogus packet, option fields too long.");
			return NULL;
		}
...
int add_option_string(uint8_t *optionptr, uint8_t *string)
{
	...
	if (end + string[OPT_LEN] + 2 + 1 >= 308) {
		LOG_DEBUG("Option 0x%02x did not fit into the packet!", string[OPT_CODE]);
		return 0;
	}
	LOG_DEBUG("Adding option 0x%02x", string[OPT_CODE]);
...
int add_simple_option(uint8_t *optionptr, uint8_t code, uint32_t data)
{
	...
	if (!length) {
		LOG_DEBUG("Could not add option 0x%02x", code);
		return 0;
	}
...
void attach_option(struct option_set **opt_list, struct dhcp_option *option, char *buffer, int length)
{
	...
	if ((existing = find_option(*opt_list, option->code))) {
		LOG_DEBUG("Attaching option (%s) to existing member of list", option->name);
		...
		LOG_DEBUG("Attaching option (%s) to list", option->name);
