...
#undef MB_CUR_MAX
#define MB_CUR_MAX 3 /* added by gengxianhu for UTF-8 max character bytes */

/* 
  * added by gengxianhu for UCS2 to UTF8 convert 
  * we don't have locale on mips board
  * this can correctly suitable for ntfs_ucstombs
  */
int myUcs2toUtf8 (char *s, wchar_t wc)
{
	int convNum = 0;
	...
	else
	{
		/* 3 bytes UTF-8 */
		*s = (char) (wc >> 12) | 0xe0;
		*(s + 1) = (char) ((wc & 0x0fc0) >>6) | 0x80;
		*(s + 2) = (char) (wc & 0x003f) | 0x80;
		convNum = 3;
	}
	return convNum;
}
...
/* 
  * added by gengxianhu for  UTF8 to UCS2 convert 
  * we don't have locale on mips board
  * this can correctly suitable for ntfs_mbstoucs
  */
int	myUtf8toUcs2 (wchar_t *pwc, const char *s, size_t n)
{
	int convNum = 0;
	const char * in = s;
	wchar_t tmp;
	...
	if (0x00 == (*in & 0x80))
	{
		...
		*pwc = *in;
		convNum = 1;
			...
			&& 0x80 == (*(in + 1) & 0xc0))
	{
		/* 2 bytes UTF-8 characters */
		tmp = (wchar_t) (*in & 0x1f);
		tmp <<= 6;
		tmp |= (wchar_t) (*(in + 1) & 0x3f);
		*pwc = tmp;
		convNum = 2;
	}
	else if (n >= 3
			...
			&& 0x80 == (*(in + 2) & 0xc0))
	{
		/* 3 bytes UTF-8 characters */
		tmp = (wchar_t) (*in & 0x0f);
		tmp <<= 6;
		tmp |= (wchar_t) (*(in + 1) & 0x3f);
		tmp <<= 6;
		tmp |= (wchar_t) (*(in + 2) & 0x3f);
		*pwc = tmp;
		convNum = 3;
	}
	else
...
#else
	//wctomb(NULL, 0);
	/* modified by gengxianhu */
	memset (mbs, 0, mbs_len);
#endif
...
int ntfs_mbstoucs(const char *ins, ntfschar **outs)
{
	...
	//mbtowc(NULL, NULL, 0);
	/* modified by gengxianhu */
	memset(ucs, 0, ucs_len * sizeof(ntfschar));
#endif
	for (i = o = cnt = 0; i < ins_size; i += cnt, o++) {
		...
		/* modified by gengxianhu */
		cnt = myUtf8toUcs2(&wc, ins + i, ins_size -i);
#endif
		if (!cnt)
			break;
		if (cnt == -1)
