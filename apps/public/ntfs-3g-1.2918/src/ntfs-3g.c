...
static void create_dev_fuse(void)
{
	...
	mknod_dev_fuse("/var/dev/fuse"); /* modified by zjj, only var dir support write permission which inludes mknod */
...
#ifdef __UCLIBC__
	{
		...
		//mknod_dev_fuse("/var/dev/misc/fuse"); /* noted by zjj, it will create fuse before mount device */
	}
#endif
}
